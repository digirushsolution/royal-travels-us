<?php
namespace MyTravelElementor\Modules\Posts\Data;

use Elementor\Utils;
use MyTravelElementor\Data\Base\Controller as Controller_Base;
...
class Controller extends Controller_Base {
	...
	public function get_items( $request ) {
		...
		if ( ! $document ) {
			return new \WP_Error(
				'document_not_exist',
				esc_html__( 'Document doesn\'t exist', 'mytravel-elementor' ),
				[ 'status' => 404 ]
			);
		}
		...
		if ( empty( $posts_widget ) ) {
			return new \WP_Error(
				'Element_not_exist',
				esc_html__( 'Posts widget doesn\'t exist', 'mytravel-elementor' ),
				[ 'status' => 404 ]
			);
		}
		 ...
		 *
		 * @var \SiliconElementor\Modules\Posts\Widgets\Posts $element_instance The Element instance.
		 */
