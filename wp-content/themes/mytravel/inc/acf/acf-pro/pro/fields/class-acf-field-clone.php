	...
	class acf_field_clone extends acf_field {
		...
		*/
		...
		function initialize() {
			...
			// vars
			$this->name          = 'clone';
			$this->label         = _x( 'Clone', 'noun', 'mytravel' );
	...
	<tbody>
		...
		*/
		...
		function render_field_settings( $field ) {
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Fields', 'mytravel' ),
					'instructions' => __( 'Select one or more fields you wish to clone', 'mytravel' ),
					'type'         => 'select',
					'name'         => 'clone',
					'multiple'     => 1,
					'allow_null'   => 1,
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Display', 'mytravel' ),
					'instructions' => __( 'Specify the style used to render the clone field', 'mytravel' ),
					'type'         => 'select',
					'name'         => 'display',
					'class'        => 'setting-display',
					'choices'      => array(
						'group'    => __( 'Group (displays selected fields in a group within this field)', 'mytravel' ),
						'seamless' => __( 'Seamless (replaces this field with selected fields)', 'mytravel' ),
					),
				)
			);
			...
			acf_render_field_setting(
				...
				array(
					...
					'instructions' => __( 'Specify the style used to render the selected fields', 'mytravel' ),
					'type'         => 'radio',
					'name'         => 'layout',
					'layout'       => 'horizontal',
					'choices'      => array(
			...
			// prefix_label
			$instructions = __( 'Labels will be displayed as %s', 'mytravel' );
			$instructions = sprintf( $instructions, '<code class="prefix-label-code-1"></code>' );
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Prefix Field Labels', 'mytravel' ),
			...
			// prefix_name
			$instructions = __( 'Values will be saved as %s', 'mytravel' );
			$instructions = sprintf( $instructions, '<code class="prefix-name-code-1"></code>' );
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Prefix Field Names', 'mytravel' ),
					'instructions' => $instructions,
					'name'         => 'prefix_name',
		...
		*/
		...
		function get_clone_setting_field_choice( $field ) {
			...
			if ( ! $field ) {
				return __( 'Unknown field', 'mytravel' );
			}

			// title
			$title = $field['label'] ? $field['label'] : __( '(no title)', 'mytravel' );
		...
		*/
		...
		function get_clone_setting_group_choice( $field_group ) {
			...
			if ( ! $field_group ) {
				return __( 'Unknown field group', 'mytravel' );
			}

			// return
			return sprintf( __( 'All fields from %s field group', 'mytravel' ), $field_group['title'] );
		}
