...
if ( ! class_exists( 'acf_field_repeater' ) ) :
	...
	class acf_field_repeater extends acf_field {
		...
		public function initialize() {
			$this->name          = 'repeater';
			$this->label         = __( 'Repeater', 'mytravel' );
		...
		public function input_admin_enqueue_scripts() {
			acf_localize_text(
				array(
					'Minimum rows not reached ({min} rows)' => __( 'Minimum rows not reached ({min} rows)', 'mytravel' ),
					'Maximum rows reached ({max} rows)' => __( 'Maximum rows reached ({max} rows)', 'mytravel' ),
					'Error loading page'                => __( 'Error loading page', 'mytravel' ),
					'Order will be assigned upon save'  => __( 'Order will be assigned upon save', 'mytravel' ),
				)
			);
		}
		...
		public function load_field( $field ) {
			...
			if ( '' === $field['button_label'] ) {
				$field['button_label'] = __( 'Add Row', 'mytravel' );
			}
		...
		function render_field_settings( $field ) {
			...
			<div class="acf-field acf-field-setting-sub_fields" data-setting="repeater" data-name="sub_fields">
				<div class="acf-label">
					<label><?php _e( 'Sub Fields', 'mytravel' ); ?></label>
			...
			if ( $supports_pagination ) {
				acf_render_field_setting(
					...
					array(
						...
						'instructions' => __( 'Useful for fields with a large number of rows.', 'mytravel' ),
				...
				acf_render_field_setting(
					$field,
					array(
						'label'        => __( 'Rows Per Page', 'mytravel' ),
						'instructions' => __( 'Set the number of rows to be displayed on a page.', 'mytravel' ),
		...
		function render_field_validation_settings( $field ) {
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Minimum Rows', 'mytravel' ),
					'instructions' => '',
					'type'         => 'number',
					'name'         => 'min',
					'placeholder'  => '0',
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Maximum Rows', 'mytravel' ),
					'instructions' => '',
					'type'         => 'number',
					'name'         => 'max',
					'placeholder'  => '0',
		...
		function render_field_presentation_settings( $field ) {
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Collapsed', 'mytravel' ),
					'instructions' => __( 'Select a sub field to show when row is collapsed', 'mytravel' ),
					'type'         => 'select',
					'name'         => 'collapsed',
					'allow_null'   => 1,
					'choices'      => $choices,
			...
			acf_render_field_setting(
				...
				array(
					...
					'type'         => 'text',
					'name'         => 'button_label',
					'placeholder'  => __( 'Add Row', 'mytravel' ),
				)
			);
		}
		...
		function validate_value( $valid, $value, $field, $input ) {
			...
			if ( empty( $field['pagination'] ) && $min && $count < $min ) {
				...
				// create error
				$error = __( 'Minimum rows not reached ({min} rows)', 'mytravel' );
		...
		public function ajax_get_rows() {
			if ( ! acf_verify_ajax() ) {
				$error = array( 'error' => __( 'Invalid nonce.', 'mytravel' ) );
			...
			if ( '' === $args['field_name'] || '' === $args['field_key'] ) {
				$error = array( 'error' => __( 'Invalid field key or name.', 'mytravel' ) );
			...
			if ( ! $field || ! $post_id ) {
				$error = array( 'error' => __( 'There was an error retrieving the field.', 'mytravel' ) );
				wp_send_json_error( $error, 404 );
			}
