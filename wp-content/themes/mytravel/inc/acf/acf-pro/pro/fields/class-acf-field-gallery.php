	...
	class acf_field_gallery extends acf_field {
		...
		*/
		...
		function initialize() {
			...
			// vars
			$this->name          = 'gallery';
			$this->label         = __( 'Gallery', 'mytravel' );
			$this->category      = 'content';
			$this->description   = __( 'An interactive interface for managing a collection of attachments, such as images.', 'mytravel' );
		...
		*/
		...
		function input_admin_enqueue_scripts() {
			...
			acf_localize_text(
				array(
					'Add Image to Gallery'      => __( 'Add Image to Gallery', 'mytravel' ),
					'Maximum selection reached' => __( 'Maximum selection reached', 'mytravel' ),
				)
			);
		}
		...
		*/
		...
		function render_attachment( $id, $field ) {
			...
			if ( $attachment['type'] === 'audio' ) {
				$dimensions = __( 'Length', 'mytravel' ) . ': ' . $attachment['fileLength'];
		...
		<div class="acf-gallery-side-info">
			...
			<p class="actions">
				<a href="#" class="acf-gallery-edit" data-id="<?php echo esc_attr( $id ); ?>"><?php _e( 'Edit', 'mytravel' ); ?></a>
				<a href="#" class="acf-gallery-remove" data-id="<?php echo esc_attr( $id ); ?>"><?php _e( 'Remove', 'mytravel' ); ?></a>
			</p>
		</div>
...
<div <?php echo acf_esc_attrs( $attrs ); ?>>
	...
	<div class="acf-gallery-main">
			...
			<?php if ( $attachments ) : ?>
				...
				foreach ( $attachments as $i => $attachment ) :
					...
					<div class="<?php echo esc_attr( $a_class ); ?>" data-id="<?php echo esc_attr( $a_id ); ?>">
						...
						<div class="actions">
							<a class="acf-icon -cancel dark acf-gallery-remove" href="#" data-id="<?php echo esc_attr( $a_id ); ?>" title="<?php _e( 'Remove', 'mytravel' ); ?>"></a>
						</div>
					</div>
				<?php endforeach; ?>
			<?php endif; ?>
		...
		<div class="acf-gallery-toolbar">
			<ul class="acf-hl">
				<li>
					<a href="#" class="acf-button button button-primary acf-gallery-add"><?php _e( 'Add to gallery', 'mytravel' ); ?></a>
				</li>
				<li class="acf-fr">
					<select class="acf-gallery-sort">
						<option value=""><?php _e( 'Bulk actions', 'mytravel' ); ?></option>
						<option value="date"><?php _e( 'Sort by date uploaded', 'mytravel' ); ?></option>
						<option value="modified"><?php _e( 'Sort by date modified', 'mytravel' ); ?></option>
						<option value="title"><?php _e( 'Sort by title', 'mytravel' ); ?></option>
						<option value="reverse"><?php _e( 'Reverse current order', 'mytravel' ); ?></option>
					</select>
				</li>
			</ul>
		</div>
	...
	<div class="acf-gallery-side">
		<div class="acf-gallery-side-inner">
			...
			<div class="acf-gallery-toolbar">
				<ul class="acf-hl">
					<li>
						<a href="#" class="acf-button button acf-gallery-close"><?php _e( 'Close', 'mytravel' ); ?></a>
					</li>
					<li class="acf-fr">
						<a class="acf-button button button-primary acf-gallery-update" href="#"><?php _e( 'Update', 'mytravel' ); ?></a>
					</li>
				</ul>
			</div>
		</div>	
		...
		*/
		...
		function render_field_settings( $field ) {
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Return Format', 'mytravel' ),
					'instructions' => '',
					'type'         => 'radio',
					'name'         => 'return_format',
					'layout'       => 'horizontal',
					'choices'      => array(
						'array' => __( 'Image Array', 'mytravel' ),
						'url'   => __( 'Image URL', 'mytravel' ),
						'id'    => __( 'Image ID', 'mytravel' ),
					),
				)
			);
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Library', 'mytravel' ),
					'instructions' => __( 'Limit the media library choice', 'mytravel' ),
					'type'         => 'radio',
					'name'         => 'library',
					'layout'       => 'horizontal',
					'choices'      => array(
						'all'        => __( 'All', 'mytravel' ),
						'uploadedTo' => __( 'Uploaded to post', 'mytravel' ),
					),
				)
			);
		}
		...
		function render_field_validation_settings( $field ) {
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Minimum Selection', 'mytravel' ),
					'instructions' => '',
					'type'         => 'number',
					'name'         => 'min',
				)
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Maximum Selection', 'mytravel' ),
					'instructions' => '',
					'type'         => 'number',
					'name'         => 'max',
				)
			...
			acf_render_field_setting(
				$field,
				array(
					'label'   => __( 'Minimum', 'mytravel' ),
					'hint'    => __( 'Restrict which images can be uploaded', 'mytravel' ),
					'type'    => 'text',
					'name'    => 'min_width',
					'prepend' => __( 'Width', 'mytravel' ),
					'append'  => 'px',
				)
			);
			...
			acf_render_field_setting(
				$field,
				array(
					'label' => __( 'Allowed File Types', 'mytravel' ),
					'hint'  => __( 'Comma separated list. Leave blank for all types', 'mytravel' ),
					'type'  => 'text',
					'name'  => 'mime_types',
				)
			);
		...
		function render_field_presentation_settings( $field ) {
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Insert', 'mytravel' ),
					'instructions' => __( 'Specify where new attachments are added', 'mytravel' ),
					'type'         => 'select',
					'name'         => 'insert',
					'choices'      => array(
						'append'  => __( 'Append to the end', 'mytravel' ),
						'prepend' => __( 'Prepend to the beginning', 'mytravel' ),
					),
				)
			);
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Preview Size', 'mytravel' ),
					'instructions' => '',
					'type'         => 'select',
					'name'         => 'preview_size',
		...
		*/
		...
		function validate_value( $valid, $value, $field, $input ) {
			...
			if ( count( $value ) < $field['min'] ) {
				$valid = _n( '%1$s requires at least %2$s selection', '%1$s requires at least %2$s selections', $field['min'], 'mytravel' );
