	...
	class acf_field_flexible_content extends acf_field {
		...
		*/
		...
		function initialize() {
			...
			$this->label         = __( 'Flexible Content', 'mytravel' );
			$this->category      = 'layout';
			$this->description   = __( 'Allows you to define, create and manage content with total control by creating layouts that contain subfields that content editors can choose from.', 'mytravel' ) . ' ' . __( 'We do not recommend using this field in ACF Blocks.', 'mytravel' );
			...
			$this->defaults      = array(
				...
				'min'          => '',
				'max'          => '',
				'button_label' => __( 'Add Row', 'mytravel' ),
			);
		...
		function input_admin_enqueue_scripts() {
			...
			acf_localize_text(
				array(
					...
					'layouts' => __( 'layouts', 'mytravel' ),
					'Fields'  => __( 'Fields', 'mytravel' ),

					// min / max
					'This field requires at least {min} {label} {identifier}' => __( 'This field requires at least {min} {label} {identifier}', 'mytravel' ),
					'This field has a limit of {max} {label} {identifier}' => __( 'This field has a limit of {max} {label} {identifier}', 'mytravel' ),

					// popup badge
					'{available} {label} {identifier} available (max {max})' => __( '{available} {label} {identifier} available (max {max})', 'mytravel' ),
					'{required} {label} {identifier} required (min {min})' => __( '{required} {label} {identifier} required (min {min})', 'mytravel' ),

					// field settings
					'Flexible Content requires at least 1 layout' => __( 'Flexible Content requires at least 1 layout', 'mytravel' ),
				)
			);
		}
		...
		*/
		...
		function render_field( $field ) {
			...
			// no value message
			$no_value_message = __( 'Click the "%s" button below to start creating your layout', 'mytravel' );
...
<div <?php echo acf_esc_attr( $div ); ?>>
	...
	<div class="acf-fc-layout-handle" title="<?php _e( 'Drag to reorder', 'mytravel' ); ?>" data-name="collapse-layout"><?php echo acf_esc_html( $title ); ?></div>

	<div class="acf-fc-layout-controls">
		<a class="acf-icon -plus small light acf-js-tooltip" href="#" data-name="add-layout" title="<?php _e( 'Add layout', 'mytravel' ); ?>"></a>
		<a class="acf-icon -duplicate small light acf-js-tooltip" href="#" data-name="duplicate-layout" title="<?php _e( 'Duplicate layout', 'mytravel' ); ?>"></a>
		<a class="acf-icon -minus small light acf-js-tooltip" href="#" data-name="remove-layout" title="<?php _e( 'Remove layout', 'mytravel' ); ?>"></a>
		<a class="acf-icon -collapse small -clear acf-js-tooltip" href="#" data-name="collapse-layout" title="<?php _e( 'Click to toggle', 'mytravel' ); ?>"></a>
	</div>
	...
	<?php endif; ?>
				...
				<div class="acf-field acf-field-setting-fc_layout" data-name="fc_layout" data-setting="flexible_content" data-layout-label="<?php echo esc_attr( $layout['label'] ); ?>" data-layout-name="<?php echo esc_attr( $layout['name'] ); ?>" data-id="<?php echo esc_attr( $layout['key'] ); ?>">
					<div class="acf-label acf-field-settings-fc_head">
						<div class="acf-fc_draggable">
							<label class="acf-fc-layout-label reorder-layout"><?php esc_attr_e( 'Layout', 'mytravel' ); ?></label>
						</div>
						...
						<ul class="acf-bl acf-fl-actions">
							<li><button class="acf-btn acf-btn-tertiary acf-btn-sm acf-field-setting-fc-delete"><i class="acf-icon acf-icon-trash delete-layout " href="#" title="<?php esc_attr_e( 'Delete Layout', 'mytravel' ); ?>"></i></button></li>
							<li><button class="acf-btn acf-btn-tertiary acf-btn-sm acf-field-setting-fc-duplicate"><i class="acf-icon -duplicate duplicate-layout" href="#" title="<?php esc_attr_e( 'Duplicate Layout', 'mytravel' ); ?>"></i></button></li>
							<li class="acf-fc-add-layout"><button class="add-layout acf-btn acf-btn-primary add-field" href="#" title="<?php esc_attr_e( 'Add New Layout', 'mytravel' ); ?>"><i class="acf-icon acf-icon-plus"></i><?php esc_html_e( 'Add Layout', 'mytravel' ); ?></button></li>
					...
					<ul class="acf-fc-meta acf-bl">
						...
						<li class="acf-fc-meta-display acf-fc-meta-left">
							<div class="acf-input-prepend"><?php esc_html_e( 'Layout', 'mytravel' ); ?></div>
		...
		function render_field_presentation_settings( $field ) {
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Minimum Layouts', 'mytravel' ),
					'instructions' => '',
					'type'         => 'number',
					'name'         => 'min',
				)
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Maximum Layouts', 'mytravel' ),
					'instructions' => '',
					'type'         => 'number',
					'name'         => 'max',
				)
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Button Label', 'mytravel' ),
					'instructions' => '',
					'type'         => 'text',
					'name'         => 'button_label',
				)
		...
		*/
		...
		function validate_value( $valid, $value, $field, $input ) {
			...
			if ( $min && $count < $min ) {
				...
				// vars
				$error      = __( 'This field requires at least {min} {label} {identifier}', 'mytravel' );
				$identifier = _n( 'layout', 'layouts', $min, 'mytravel' );
		...
		*/
		...
		public function validate_rest_value( $valid, $value, $field ) {
			...
			if ( ! is_array( $value ) && is_null( $value ) ) {
				$error = sprintf( __( '%s must be of type array or null.', 'mytravel' ), $param );
