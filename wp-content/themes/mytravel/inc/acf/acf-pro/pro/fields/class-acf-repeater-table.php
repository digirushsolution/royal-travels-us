...
class ACF_Repeater_Table {
	...
	public function row_handle( $i ) {
		...
		$title        = __( 'Drag to reorder', 'mytravel' );
		$row_num_html = sprintf(
			'<span class="acf-row-number" title="%s">%d</span>',
			__( 'Click to reorder', 'mytravel' ),
			$hr_row_num
		);
		...
		<td class="<?php echo $classes; ?>" title="<?php echo $title; ?>">
			<?php if ( $this->field['collapsed'] ) : ?>
				<a class="acf-icon -collapse small" href="#" data-event="collapse-row" title="<?php _e( 'Click to toggle', 'mytravel' ); ?>"></a>
			<?php endif; ?>
	...
	public function row_actions() {
		...
		<td class="acf-row-handle remove">
			<a class="acf-icon -plus small acf-js-tooltip hide-on-shift" href="#" data-event="add-row" title="<?php _e( 'Add row', 'mytravel' ); ?>"></a>
			<a class="acf-icon -duplicate small acf-js-tooltip show-on-shift" href="#" data-event="duplicate-row" title="<?php _e( 'Duplicate row', 'mytravel' ); ?>"></a>
			<a class="acf-icon -minus small acf-js-tooltip" href="#" data-event="remove-row" title="<?php _e( 'Remove row', 'mytravel' ); ?>"></a>
		</td>
		<?php
	}
	...
	public function pagination() {
		...
		$html_current_page = sprintf(
			...
			'<label for="current-page-selector" class="screen-reader-text">' . __( 'Current Page', 'mytravel' ) . '</label>',
			1,
			strlen( $total_pages )
		);
		...
		<div class="acf-tablenav tablenav-pages">
			<a class="first-page button acf-nav" aria-hidden="true" data-event="first-page" title="<?php esc_attr_e( 'First Page', 'mytravel' ); ?>">
				<span class="screen-reader-text"><?php esc_html_e( 'First Page', 'mytravel' ); ?></span>
				<span aria-hidden="true">&laquo;</span>
			</a>
			<a class="prev-page button acf-nav" aria-hidden="true" data-event="prev-page" title="<?php esc_attr_e( 'Previous Page', 'mytravel' ); ?>">
				<span class="screen-reader-text"><?php esc_html_e( 'Previous Page', 'mytravel' ); ?></span>
				<span aria-hidden="true">&lsaquo;</span>
			</a>
			<span class="paging-input">
				<label for="current-page-selector" class="screen-reader-text"><?php esc_html_e( 'Current Page', 'mytravel' ); ?></label>
				<span class="tablenav-paging-text" title="<?php esc_attr_e( 'Current Page', 'mytravel' ); ?>">
				<?php
				printf(
					/* translators: 1: Current page, 2: Total pages. */
					_x( '%1$s of %2$s', 'paging','mytravel' ),
					$html_current_page,
					$html_total_pages
				);
				?>
			...
			<a class="next-page button acf-nav" data-event="next-page" title="<?php esc_attr_e( 'Next Page', 'mytravel' ); ?>">
				<span class="screen-reader-text"><?php esc_html_e( 'Next Page', 'mytravel' ); ?></span>
				<span aria-hidden="true">&rsaquo;</span>
			</a>
			<a class="last-page button acf-nav" data-event="last-page" title="<?php esc_attr_e( 'Last Page', 'mytravel' ); ?>">
				<span class="screen-reader-text"><?php esc_html_e( 'Last Page', 'mytravel' ); ?></span>
