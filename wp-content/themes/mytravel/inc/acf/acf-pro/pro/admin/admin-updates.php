	...
	class ACF_Admin_Updates {
		...
		function display_wp_error( $wp_error ) {
			...
			acf_new_admin_notice(
				array(
					'text' => __( '<strong>Error</strong>. Could not connect to the update server', 'mytravel' ) . ' <span class="description">(' . esc_html( $wp_error->get_error_message() ) . ').</span>',
					'type' => 'error',
				)
			);
		}
		...
		function admin_menu() {
			...
			// Add submenu.
			$page = add_submenu_page( 'edit.php?post_type=acf-field-group', __( 'Updates', 'mytravel' ), __( 'Updates', 'mytravel' ), acf_get_setting( 'capability' ), 'acf-settings-updates', array( $this, 'html' ) );
			...
			if ( version_compare( $info['version'], $version, '>' ) ) {
				...
				if ( $no_update && ! empty( $no_update['reason'] ) && $no_update['reason'] === 'wp_not_compatible' ) {
					...
					acf_new_admin_notice(
						array(
							...
							'text' => sprintf( __( 'An update to ACF is available, but it is not compatible with your version of WordPress. Please upgrade to WordPress %s or newer to update ACF.', 'mytravel' ), $no_update['requires'] ),
							'type' => 'error',
						)
					);
				}
				...
				if ( $license ) {
					if ( isset( $update['license_valid'] ) && ! $update['license_valid'] ) {
						...
						acf_new_admin_notice(
							array(
								'text' => __( '<strong>Error</strong>. Your license for this site has expired or been deactivated. Please reactivate your ACF PRO license.', 'mytravel' ),
								'type' => 'error',
							)
						);
					} else {
						...
						if ( $update && ! $update['package'] ) {
							...
							acf_new_admin_notice(
								array(
									'text' => __( '<strong>Error</strong>. Could not authenticate update package. Please check again or deactivate and reactivate your ACF PRO license.', 'mytravel' ),
									'type' => 'error',
								)
							);
						}
