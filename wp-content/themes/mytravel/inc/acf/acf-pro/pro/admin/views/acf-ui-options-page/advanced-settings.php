	...
	switch ( $tab_key ) {
		case 'visibility':
			...
			/* translators: %s = "dashicon class name", link to the WordPress dashicon documentation. */
				__( 'The icon used for the options page menu item in the admin dashboard. Can be a URL or %s to use for the icon.', 'mytravel' ),
				$acf_dashicon_link
			);
			...
			$acf_menu_position_link = sprintf(
				...
				__( 'Learn more about menu positions.', 'mytravel' )
			);
			$acf_menu_position_desc = sprintf(
				/* translators: %s - link to WordPress docs to learn more about menu positions. */
				__( 'The position in the menu where this page should appear. %s', 'mytravel' ),
			...
			$acf_menu_position_desc_child = __( 'The position in the menu where this child page should appear. The first child page is 0, the next is 1, etc.', 'mytravel' );
			...
			acf_render_field_wrap(
				array(
					'label'        => __( 'Redirect to Child Page', 'mytravel' ),
					'instructions' => __( 'When child pages exist for this parent page, this page will redirect to the first child page.', 'mytravel' ),
					'type'         => 'true_false',
					'name'         => 'redirect',
					'key'          => 'redirect',
					'prefix'       => 'acf_ui_options_page',
			...
			acf_render_field_wrap(
				array(
					...
					'instructions' => __( 'A descriptive summary of the options page.', 'mytravel' ),
				),
				'div',
				'field'
			);
		...
		case 'labels':
			acf_render_field_wrap(
				array(
					'label'        => __( 'Update Button Label', 'mytravel' ),
					'instructions' => __( 'The label used for the submit button which updates the fields on the options page.', 'mytravel' ),
					'placeholder'  => __( 'Update', 'mytravel' ),
					'type'         => 'text',
					'name'         => 'update_button',
					'key'          => 'update_button',
					'prefix'       => 'acf_ui_options_page',
			...
			acf_render_field_wrap(
				array(
					'label'        => __( 'Updated Message', 'mytravel' ),
					'instructions' => __( 'The message that is displayed after successfully updating the options page.', 'mytravel' ),
					'placeholder'  => __( 'Updated Options', 'mytravel' ),
					'type'         => 'text',
		...
		case 'permissions':
			...
			acf_render_field_wrap(
				array(
					...
					'label'        => __( 'Capability', 'mytravel' ),
					'instructions' => __( 'The capability required for this menu to be displayed to the user.', 'mytravel' ),
			...
			acf_render_field_wrap(
				array(
					...
					'label'        => __( 'Data Storage', 'mytravel' ),
					'instructions' => __( 'By default, the option page stores field data in the options table. You can make the page load field data from a post, user, or term.', 'mytravel' ),
					'choices'      => array(
						'options' => __( 'Options', 'mytravel' ),
						'post_id' => __( 'Custom Storage', 'mytravel' ),
					),
					'default'      => 'options',
					'hide_search'  => true,
			...
			$acf_custom_storage_link = sprintf(
				...
				__( 'Learn more about available settings.', 'mytravel' )
			);

			$acf_custom_storage_desc = sprintf(
				/* translators: %s = link to learn more about storage locations. */
				__( 'Set a custom storage location. Can be a numeric post ID (123), or a string (`user_2`). %s', 'mytravel' ),
				$acf_custom_storage_link
			);

			acf_render_field_wrap(
				array(
					'label'        => __( 'Custom Storage', 'mytravel' ),
			...
			acf_render_field_wrap(
				array(
					'label'        => __( 'Autoload Options', 'mytravel' ),
					'instructions' => __( 'Improve performance by loading the fields in the option records automatically when WordPress loads.', 'mytravel' ),
					'type'         => 'true_false',
					'name'         => 'autoload',
					'key'          => 'autoload',
					'prefix'       => 'acf_ui_options_page',
