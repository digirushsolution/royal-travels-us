...
$activate_deactivate_btn = $active ? __( 'Deactivate License', 'mytravel' ) : __( 'Activate License', 'mytravel' );
...
function acf_pro_render_license_status_table( $status ) {
	...
	$status_text      = _x( 'Inactive', 'license status', 'mytravel' );
	$is_lifetime      = ! empty( $status['lifetime'] );

	if ( 'active' === $status['status'] ) {
		$status_text = _x( 'Active', 'license status', 'mytravel' );
	} elseif ( 'expired' === $status['status'] ) {
		$status_text = _x( 'Expired', 'license status', 'mytravel' );
	} elseif ( 'cancelled' === $status['status'] ) {
		$status_text = _x( 'Cancelled', 'license status', 'mytravel' );
	}
	...
	<table class="acf-license-status-table">
		<tr>
			<th>
				...
				if ( $is_lifetime || 'inactive' === $status['status'] ) {
					esc_html_e( 'License Status', 'mytravel' );
				} else {
					esc_html_e( 'Subscription Status', 'mytravel' );
				}
				?>
			</th>
		...
		<tr>
			<th>
				<?php
				if ( $is_lifetime ) {
					esc_html_e( 'License Type', 'mytravel' );
				} else {
					esc_html_e( 'Subscription Type', 'mytravel' );
				}
				?>
			</th>
			<td>
		...
		<tr>
			<th>
				...
				if ( acf_pro_is_license_expired( $status ) ) {
					esc_html_e( 'Subscription Expired', 'mytravel' );
				} else {
					esc_html_e( 'Subscription Expires', 'mytravel' );
				}
				?>
			</th>
			<td>
...
function acf_pro_render_manage_license_button( $status ) {
	...
	$text  = __( 'Manage License', 'mytravel' );
	$class = '';

	if ( acf_pro_is_license_expired( $status ) ) {
		$text  = __( 'Renew Subscription', 'mytravel' );
...
<div class="wrap acf-settings-wrap acf-updates">
	...
	<h1><?php esc_html_e( 'Updates', 'mytravel' ); ?></h1>

	<div class="acf-box" id="acf-license-information">
		<div class="title">
			<h3><?php esc_html_e( 'License Information', 'mytravel' ); ?></h3>
		</div>
		<div class="inner">
			<?php if ( $is_defined_license ) : ?>

				<p class="acf-license-defined">
					<?php echo acf_esc_html( apply_filters( 'acf/admin/license_key_constant_message', __( 'Your license key is defined in wp-config.php.', 'mytravel' ) ) ); ?>
				</p>
				...
				<?php if ( ! $active ) : ?>
					<div class="acf-retry-activation">
						...
						$acf_recheck_text  = __( 'Recheck License', 'mytravel' );
						printf(
			...
			<?php else : // License is not defined. ?>
				<form action="" method="post" class="acf-activation-form">
					...
					<label for="acf-field-acf_pro_license"><?php esc_html_e( 'License Key', 'mytravel' ); ?></label>
					<?php
					acf_render_field(
						array(
							'type'     => 'text',
			...
			<div class="acf-license-status-wrap">
				...
				if ( ! $active && ! defined( 'ACF_PRO_LICENSE' ) ) :
					...
					<div class="acf-no-license-view-pricing">
						<span>
							<?php
							$acf_view_pricing_text = esc_html__( 'View pricing & purchase', 'mytravel' );
							...
							/* translators: %s - link to ACF website */
								__( 'Don\'t have an ACF PRO license? %s', 'mytravel' ),
	...
	<div class="acf-box" id="acf-update-information">
		<div class="title">
			<h3><?php esc_html_e( 'Update Information', 'mytravel' ); ?></h3>
		</div>
		<div class="inner">
			<table class="form-table">
				<tbody>
					<tr>
						<th>
							<label><?php esc_html_e( 'Current Version', 'mytravel' ); ?></label>
						</th>
						<td>
					...
					<tr>
						<th>
							<label><?php esc_html_e( 'Latest Version', 'mytravel' ); ?></label>
						</th>
						<td>
					...
					<tr>
						<th>
							<label><?php esc_html_e( 'Update Available', 'mytravel' ); ?></label>
						</th>
						<td>
							<?php if ( $update_available ) : ?>

								<span style="margin-right: 5px;"><?php esc_html_e( 'Yes', 'mytravel' ); ?></span>
							<?php else : ?>
								<span style="margin-right: 5px;"><?php esc_html_e( 'No', 'mytravel' ); ?></span>
							<?php endif; ?>
						</td>
					</tr>
					...
					<tr>
						<th>
							<label><?php esc_html_e( 'Upgrade Notice', 'mytravel' ); ?></label>
						</th>
						<td>
			...
			<?php if ( $update_available ) : ?>
				<?php if ( $wp_not_compatible ) : ?>
					<a class="button" disabled="disabled" href="#"><?php esc_html_e( 'Please upgrade WordPress to update ACF', 'mytravel' ); ?></a>
				<?php elseif ( $license_error ) : ?>
					<a class="button" disabled="disabled" href="#"><?php esc_html_e( 'Please reactivate your license to unlock updates', 'mytravel' ); ?></a>
				<?php elseif ( $active && is_multisite() ) : ?>
					<a class="button" disabled="disabled" href="#"><?php esc_html_e( 'Update ACF in Network Admin', 'mytravel' ); ?></a>
				<?php elseif ( $active ) : ?>
					<a class="acf-btn" href="<?php echo esc_attr( admin_url( 'plugins.php?s=Advanced+Custom+Fields+Pro' ) ); ?>"><?php esc_html_e( 'Update Plugin', 'mytravel' ); ?></a>
				<?php else : ?>
					<a class="button" disabled="disabled" href="#"><?php esc_html_e( 'Enter your license key to unlock updates', 'mytravel' ); ?></a>
				<?php endif; ?>
			<?php else : ?>
				<a class="acf-btn acf-btn-secondary" href="<?php echo esc_attr( add_query_arg( 'force-check', 1 ) ); ?>"><?php esc_html_e( 'Check For Updates', 'mytravel' ); ?></a>
			<?php endif; ?>
		</div>
	</div>
</div>
