	...
	class ACF_Admin_UI_Options_Page extends ACF_Admin_Internal_Post_Type {
		...
		public function post_updated_messages( $messages ) {
			$messages['acf-ui-options-page'] = array(
				...
				3  => __( 'Options page deleted.', 'mytravel' ),
				4  => __( 'Options page updated.', 'mytravel' ),
				5  => false, // Post type does not support revisions.
				6  => $this->options_page_created_message( true ), // Created.
				7  => __( 'Options page saved.', 'mytravel' ),
				8  => __( 'Options page submitted.', 'mytravel' ),
				9  => __( 'Options page scheduled for.', 'mytravel' ),
				10 => __( 'Options page draft updated.', 'mytravel' ),
			);
		...
		public function options_page_created_message( $created = false ) {
			...
			$item_saved_text = sprintf( __( '%s options page updated', 'mytravel' ), $title );
			/* translators: %s options page name */
			$add_fields_text = sprintf( __( 'Add fields to %s', 'mytravel' ), $title );

			if ( $created ) {
				/* translators: %s options page name */
				$item_saved_text = sprintf( __( '%s options page created', 'mytravel' ), $title );
			}
			...
			<div class="acf-item-saved-links">
				...
				<a class="acf-link-field-groups" href="#"><?php esc_html_e( 'Link existing field groups', 'mytravel' ); ?></a>
			</div>
			<?php
			return ob_get_clean();
		}
		...
		public function admin_head() {
			...
			// metaboxes.
			add_meta_box( 'acf-basic-settings', __( 'Basic Settings', 'mytravel' ), array( $this, 'mb_basic_settings' ), 'acf-ui-options-page', 'normal', 'high' );
			add_meta_box( 'acf-advanced-settings', __( 'Advanced Settings', 'mytravel' ), array( $this, 'mb_advanced_settings' ), 'acf-ui-options-page', 'normal', 'high' );
		...
		public function post_submitbox_misc_actions() {
			...
			$status_label = $acf_ui_options_page['active'] ? _x( 'Active', 'post status', 'mytravel' ) : _x( 'Inactive', 'post status', 'mytravel' );
			?>
		...
		public function get_parent_page_choices( int $post_id = 0 ) {
			...
			$acf_parent_page_choices = array( 'None' => array( 'none' => __( 'No Parent', 'mytravel' ) ) );
		...
		public function ajax_create_options_page() {
				...
				if ( ! empty( $existing_options_pages ) ) {
					foreach ( $existing_options_pages as $existing_options_page ) {
						if ( $existing_options_page['menu_slug'] === $options_page['menu_slug'] ) {
							wp_send_json_error(
								array(
									'error' => __( 'The provided Menu Slug already exists.', 'mytravel' ),
								)
							);
						}
					}
			...
			wp_send_json_success(
				array(
					'content' => $content,
					'title'   => esc_html__( 'Add New Options Page', 'mytravel' ),
				)
			);
		}
	}
