...
function acf_register_block_type( $block ) {
	...
	// Require name.
	if ( ! $block['name'] ) {
		$message = __( 'Block type name is required.', 'mytravel' );
	...
	if ( acf_has_block_type( $block['name'] ) ) {
		...
		$message = sprintf( __( 'Block type "%s" is already registered.', 'mytravel' ), $block['name'] );
		_doing_it_wrong( __FUNCTION__, $message, '5.8.0' ); //phpcs:ignore -- escape not required.
		return false;
	}
...
function acf_enqueue_block_assets() {
	...
	acf_localize_text(
		array(
			'Switch to Edit'           => __( 'Switch to Edit', 'mytravel' ),
			'Switch to Preview'        => __( 'Switch to Preview', 'mytravel' ),
			'Change content alignment' => __( 'Change content alignment', 'mytravel' ),

			/* translators: %s: Block type title */
			'%s settings'              => __( '%s settings', 'mytravel' ),
		)
	);
...
function acf_get_empty_block_form_html( $block_name ) {
	...
	$message = __( 'This block contains no editable fields.', 'mytravel' );

	if ( acf_current_user_can_admin() ) {
		$message .= ' ';
		$message .= sprintf(
			/* translators: %s: an admin URL to the field group edit screen */
			__( 'Assign a <a href="%s" target="_blank">field group</a> to add fields to this block.', 'mytravel' ),
