	...
	class acf_pro_updates {
		...
		public function modify_plugin_update_message( $plugin_data, $response ) {
			...
			if ( is_multisite() ) {
				...
				$message           = __( 'To enable updates, please enter your license key on the <a href="%1$s">Updates</a> page of the main site. If you don\'t have a license key, please see <a href="%2$s" target="_blank">details & pricing</a>.', 'mytravel' );
				$updates_page_link = get_admin_url( get_main_site_id(), 'edit.php?post_type=acf-field-group&page=acf-settings-updates' );
			} else {
				/* translators: %1 A link to the updates page. %2 link to the pricing page */
				$message           = __( 'To enable updates, please enter your license key on the <a href="%1$s">Updates</a> page. If you don\'t have a license key, please see <a href="%2$s" target="_blank">details & pricing</a>.', 'mytravel' );
...
function acf_pro_check_defined_license() {
	...
	if ( $license ) {
		...
		if ( acf_pro_get_license_key() !== ACF_PRO_LICENSE ) {
			...
			if ( is_wp_error( $deactivation_response ) ) {
				return acf_pro_set_activation_failure_transient( __( 'Your defined license key has changed, but an error occurred when connecting to activation server', 'mytravel' ) . ' <span class="description">(' . esc_html( $deactivation_response->get_error_message() ) . ').</span>', ACF_PRO_LICENSE );
				// A deactivation error occurred. Display the message returned by our API.
			} elseif ( ! $deactivation_response['success'] ) {
				return acf_pro_set_activation_failure_transient( __( 'Your defined license key has changed, but an error occurred when deactivating your old license', 'mytravel' ) . ' <span class="description">(' . $deactivation_response['message'] . ').</span>', ACF_PRO_LICENSE );
			}
		} else {
	...
	if ( $activation_response === false ) {
		...
		$error_text = __( 'An error occurred when connecting to activation server', 'mytravel' ) . ' <span class="description">(' . esc_html( $activation_response->get_error_message() ) . ').</span>';

		// A deactivation error occurred. Display the message returned by our API.
	} elseif ( ! $activation_response['success'] ) {
		$error_text  = __( 'There was an issue activating your license key.', 'mytravel' ) . ' ';
		...
		acf_add_admin_notice(
			__( '<strong>ACF PRO &mdash;</strong> Your license key has been activated successfully. Access to updates, support &amp; PRO features is now enabled.', 'mytravel' ),
			'success'
		);
...
function acf_pro_get_translated_connect_message( $text ) {
	...
	if ( strpos( $text, 'key activated' ) !== false ) {
		return __( 'Your license key has been activated successfully. Access to updates, support &amp; PRO features is now enabled.', 'mytravel' );
	} elseif ( strpos( $text, 'key deactivated' ) !== false ) {
		return __( 'Your license key has been deactivated.', 'mytravel' );
	} elseif ( strpos( $text, 'key not found' ) !== false ) {
		$text = __( 'License key not found. Make sure you have copied your license key exactly as it appears in your receipt or your account.', 'mytravel' );
		...
		$text .= sprintf(
			...
			__( 'View your licenses', 'mytravel' )
		);

		return $text;
	} elseif ( strpos( $text, 'key unavailable' ) !== false ) {
		$text = __( 'Your license key has expired and cannot be activated.', 'mytravel' );
		...
		$text .= sprintf(
			...
			__( 'View your subscriptions', 'mytravel' )
		);

		return $text;
	} elseif ( strpos( $text, 'Activation limit reached' ) !== false ) {
		$text = __( 'You have reached the activation limit for the license.', 'mytravel' );
		...
		$check_again = sprintf(
			...
			__( 'check again', 'mytravel' )
		);

		/* translators: %1$s - link to view licenses, %2$s - link to try activating license again */
		$text .= ' ' . sprintf( __( '%1$s or %2$s.', 'mytravel' ), $view_license, $check_again );

		return $text;
	} elseif ( strpos( $text, 'scheduled maintenance' ) !== false ) {
		return __( 'The ACF activation server is temporarily unavailable for scheduled maintenance. Please try again later.', 'mytravel' );
	} elseif ( strpos( $text, 'Something went wrong' ) !== false ) {
		return __( 'An internal error occurred when trying to check your license key. Please try again later.', 'mytravel' );
	}

	/* translators: %s an untranslatable internal upstream error message */
	return sprintf( __( 'An unknown error occurred while trying to validate your license: %s.', 'mytravel' ), $text );
}
...
function acf_pro_display_activation_error( $screen ) {
	...
	$activation_data['error'] = __( '<strong>ACF PRO &mdash;</strong>', 'mytravel' ) . ' ' . $activation_data['error'];
	...
	if ( strpos( $activation_data['error'], 'http' ) === false ) {
		...
		$activation_data['error'] = $activation_data['error'] . ' <a href="' . admin_url( $check_again_url . '&acf_retry_nonce=' . $nonce ) . '">' . __( 'Check again', 'mytravel' ) . '</a>';
	}
...
function display_wp_activation_error( $wp_error ) {
	...
	acf_new_admin_notice(
		array(
			'text' => __( 'Could not connect to the activation server', 'mytravel' ) . ' <span class="description">(' . esc_html( $wp_error->get_error_message() ) . ').</span>',
			'type' => 'error',
		)
	);
}
		...
		if ( is_array( $response ) ) {
			...
			if ( ! empty( $response['code'] ) && 'activation_not_found' === $response['code'] ) {
				$status['error_msg'] = sprintf(
					...
					__( 'Your license key is valid but not activated on this site. Please <a href="%s">deactivate</a> and then reactivate the license.', 'mytravel' ),
	...
	if ( is_wp_error( $reactivation ) || ! is_array( $reactivation ) || empty( $reactivation['success'] ) ) {
		...
		if ( is_array( $reactivation ) && ! empty( $reactivation['message'] ) ) {
			$license_status['error_msg'] = sprintf(
				...
				__( "Your site URL has changed since last activating your license, but we weren't able to automatically reactivate it: %s", 'mytravel' ),
		...
		acf_add_admin_notice(
			__( '<strong>ACF PRO &mdash;</strong>', 'mytravel' ) . ' ' . __( "Your site URL has changed since last activating your license. We've automatically activated it for this site URL.", 'mytravel' ),
			'success'
		);
	}
}
