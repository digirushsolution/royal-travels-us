	...
	class ACF_UI_Options_Page extends ACF_Internal_Post_Type {
		...
		public function register_post_type() {
			...
			register_post_type(
				...
				array(
					'labels'          => array(
						'name'               => __( 'Options Pages', 'mytravel' ),
						'singular_name'      => __( 'Options Pages', 'mytravel' ),
						'add_new'            => __( 'Add New', 'mytravel' ),
						'add_new_item'       => __( 'Add New Options Page', 'mytravel' ),
						'edit_item'          => __( 'Edit Options Page', 'mytravel' ),
						'new_item'           => __( 'New Options Page', 'mytravel' ),
						'view_item'          => __( 'View Options Page', 'mytravel' ),
						'search_items'       => __( 'Search Options Pages', 'mytravel' ),
						'not_found'          => __( 'No Options Pages found', 'mytravel' ),
						'not_found_in_trash' => __( 'No Options Pages found in Trash', 'mytravel' ),
					),
					'public'          => false,
					'hierarchical'    => true,
					'show_ui'         => true,
		...
		public function get_settings_array() {
			return array(
				...
				'description'            => '',
				// Labels tab.
				'update_button'          => __( 'Update', 'mytravel' ),
				'updated_message'        => __( 'Options Updated', 'mytravel' ),
				// Permissions tab.
		...
		public function ajax_validate_values() {
			...
			if ( preg_match( '/^[a-z0-9_-]*$/', $menu_slug ) !== 1 ) {
				$valid = false;
				acf_add_internal_post_type_validation_error( 'menu_slug', __( 'The menu slug must only contain lower case alphanumeric characters, underscores or dashes.', 'mytravel' ) );
			}
			...
			if ( ! empty( $duplicates ) ) {
				...
				acf_add_internal_post_type_validation_error(
					'menu_slug',
					__( 'This Menu Slug is already in use by another ACF Options Page.', 'mytravel' ),
					'acf-ui-options-page'
				);
			}
