	...
	class ACF_Post_Type extends ACF_Internal_Post_Type {
		...
		public function register_post_type() {
			...
			register_post_type(
				...
				array(
					'labels'          => array(
						'name'               => __( 'Post Types', 'mytravel' ),
						'singular_name'      => __( 'Post Type', 'mytravel' ),
						'add_new'            => __( 'Add New', 'mytravel' ),
						'add_new_item'       => __( 'Add New Post Type', 'mytravel' ),
						'edit_item'          => __( 'Edit Post Type', 'mytravel' ),
						'new_item'           => __( 'New Post Type', 'mytravel' ),
						'view_item'          => __( 'View Post Type', 'mytravel' ),
						'search_items'       => __( 'Search Post Types', 'mytravel' ),
						'not_found'          => __( 'No Post Types found', 'mytravel' ),
						'not_found_in_trash' => __( 'No Post Types found in Trash', 'mytravel' ),
					),
					'public'          => false,
					'hierarchical'    => true,
					'show_ui'         => true,
		...
		public function ajax_validate_values() {
			...
			if ( strlen( $post_type_key ) > 20 ) {
				$valid = false;
				acf_add_internal_post_type_validation_error( 'post_type', __( 'The post type key must be under 20 characters.', 'mytravel' ) );
			}

			if ( preg_match( '/^[a-z0-9_-]*$/', $post_type_key ) !== 1 ) {
				$valid = false;
				acf_add_internal_post_type_validation_error( 'post_type', __( 'The post type key must only contain lower case alphanumeric characters, underscores or dashes.', 'mytravel' ) );
			}

			if ( in_array( $post_type_key, acf_get_wp_reserved_terms(), true ) ) {
				$valid = false;
				/* translators: %s a link to WordPress.org's Reserved Terms page */
				$message = sprintf( __( 'This field must not be a WordPress <a href="%s" target="_blank">reserved term</a>.', 'mytravel' ), 'https://codex.wordpress.org/Reserved_Terms' );
				...
				if ( $duplicates ) {
					$valid = false;
					acf_add_internal_post_type_validation_error( 'post_type', __( 'This post type key is already in use by another post type in ACF and cannot be used.', 'mytravel' ) );
				} else {
					...
					if ( empty( $matches ) && post_type_exists( $post_type_key ) ) {
						$valid = false;
						acf_add_internal_post_type_validation_error( 'post_type', __( 'This post type key is already in use by another post type registered outside of ACF and cannot be used.', 'mytravel' ) );
					}
				}
			}
