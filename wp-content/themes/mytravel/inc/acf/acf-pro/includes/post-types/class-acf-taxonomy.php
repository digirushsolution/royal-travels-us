	...
	class ACF_Taxonomy extends ACF_Internal_Post_Type {
		...
		public function register_post_type() {
			...
			register_post_type(
				...
				array(
					'labels'          => array(
						'name'               => __( 'Taxonomies', 'mytravel' ),
						'singular_name'      => __( 'Taxonomies', 'mytravel' ),
						'add_new'            => __( 'Add New', 'mytravel' ),
						'add_new_item'       => __( 'Add New Taxonomy', 'mytravel' ),
						'edit_item'          => __( 'Edit Taxonomy', 'mytravel' ),
						'new_item'           => __( 'New Taxonomy', 'mytravel' ),
						'view_item'          => __( 'View Taxonomy', 'mytravel' ),
						'search_items'       => __( 'Search Taxonomies', 'mytravel' ),
						'not_found'          => __( 'No Taxonomies found', 'mytravel' ),
						'not_found_in_trash' => __( 'No Taxonomies found in Trash', 'mytravel' ),
					),
					'public'          => false,
					'hierarchical'    => true,
					'show_ui'         => true,
		...
		public function ajax_validate_values() {
			...
			if ( strlen( $taxonomy_key ) > 32 ) {
				$valid = false;
				acf_add_internal_post_type_validation_error( 'taxonomy', __( 'The taxonomy key must be under 32 characters.', 'mytravel' ) );
			}

			if ( preg_match( '/^[a-z0-9_-]*$/', $taxonomy_key ) !== 1 ) {
				$valid = false;
				acf_add_internal_post_type_validation_error( 'taxonomy', __( 'The taxonomy key must only contain lower case alphanumeric characters, underscores or dashes.', 'mytravel' ) );
			}
			...
			if ( in_array( $taxonomy_key, acf_get_wp_reserved_terms(), true ) ) {
				...
				if ( $duplicates ) {
					$valid = false;
					acf_add_internal_post_type_validation_error( 'taxonomy', __( 'This taxonomy key is already in use by another taxonomy in ACF and cannot be used.', 'mytravel' ) );
				} else {
					...
					if ( empty( $matches ) && taxonomy_exists( $taxonomy_key ) ) {
						$valid = false;
						acf_add_internal_post_type_validation_error( 'taxonomy', __( 'This taxonomy key is already in use by another taxonomy registered outside of ACF and cannot be used.', 'mytravel' ) );
					}
				}
			}
