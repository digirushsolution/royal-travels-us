...
function acf_flush_field_cache( $field ) {
	...
	wp_cache_delete( acf_cache_key( "acf_get_field_post:name:{$field['name']}" ), 'mytravel' );
	wp_cache_delete( acf_cache_key( "acf_get_field_post:key:{$field['key']}" ), 'mytravel' );

	// Flush cached array of post_ids for this field's parent.
	wp_cache_delete( acf_cache_key( "acf_get_field_posts:{$field['parent']}" ), 'mytravel' );
}
