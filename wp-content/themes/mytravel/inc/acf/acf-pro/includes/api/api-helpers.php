...
function acf_get_image_sizes() {
	...
	// vars
	$sizes = array(
		'thumbnail' => __( 'Thumbnail', 'mytravel' ),
		'medium'    => __( 'Medium', 'mytravel' ),
		'large'     => __( 'Large', 'mytravel' ),
	);
	...
	// add full end
	$sizes['full'] = __( 'Full Size', 'mytravel' );
...
function acf_validate_attachment( $attachment, $field, $context = 'prepare' ) {
	...
	if ( $file['width'] || $file['height'] ) {
		...
		if ( $file['width'] ) {
			if ( $min_width && $file['width'] < $min_width ) {
				...
				// min width
				$errors['min_width'] = sprintf( __( 'Image width must be at least %dpx.', 'mytravel' ), $min_width );
			} elseif ( $max_width && $file['width'] > $max_width ) {

				// min width
				$errors['max_width'] = sprintf( __( 'Image width must not exceed %dpx.', 'mytravel' ), $max_width );
			}
		}
		...
		if ( $file['height'] ) {
			if ( $min_height && $file['height'] < $min_height ) {
				...
				// min height
				$errors['min_height'] = sprintf( __( 'Image height must be at least %dpx.', 'mytravel' ), $min_height );
			} elseif ( $max_height && $file['height'] > $max_height ) {

				// min height
				$errors['max_height'] = sprintf( __( 'Image height must not exceed %dpx.', 'mytravel' ), $max_height );
			}
		}
	}
	...
	if ( $file['size'] ) {
		...
		if ( $min_size && $file['size'] < acf_get_filesize( $min_size ) ) {
			...
			// min width
			$errors['min_size'] = sprintf( __( 'File size must be at least %s.', 'mytravel' ), acf_format_filesize( $min_size ) );
		} elseif ( $max_size && $file['size'] > acf_get_filesize( $max_size ) ) {

			// min width
			$errors['max_size'] = sprintf( __( 'File size must not exceed %s.', 'mytravel' ), acf_format_filesize( $max_size ) );
		}
	}
	...
	if ( $file['type'] ) {
		...
		if ( ! empty( $mime_types ) && ! in_array( $file['type'], $mime_types ) ) {
			...
			if ( count( $mime_types ) > 1 ) {
				...
				$mime_types[] = $last2 . ' ' . __( 'or', 'mytravel' ) . ' ' . $last1;
			}

			$errors['mime_types'] = sprintf( __( 'File type must be %s.', 'mytravel' ), implode( ', ', $mime_types ) );
		}
	}
