...
function get_field( $selector, $post_id = false, $format_value = true, $escape_html = false ) {
	...
	if ( ! $dummy_field && ! $format_value && $escape_html ) {
		_doing_it_wrong( __FUNCTION__, __( 'Returning an escaped HTML value is only possible when format_value is also true. The field value has not been returned for security.', 'mytravel' ), '6.2.6' ); //phpcs:ignore -- escape not required.
		return false;
	}
...
function get_fields( $post_id = false, $format_value = true, $escape_html = false ) {
	...
	// escape html is only compatible when formatting the value too
	if ( ! $format_value && $escape_html ) {
		_doing_it_wrong( __FUNCTION__, __( 'Returning escaped HTML values is only possible when format_value is also true. The field values have not been returned for security.', 'mytravel' ), '6.2.6' ); //phpcs:ignore -- escape not required.
		return false;
	}
...
function acf_shortcode( $atts ) {
	...
	if ( is_preview() && ! current_user_can( $preview_capability ) ) {
		return apply_filters( 'acf/shortcode/preview_capability_message', __( '[ACF shortcode value disabled for preview]', 'mytravel' ) );
	}
