	...
	foreach ( $valid_fields as $field_group_name => $fields ) {
		...
		foreach ( $fields as $key => $data ) {
			$field_group['children'][] = array(
				...
				'human_field_type' => sprintf( __( '%s Field', 'mytravel' ), acf_get_field_type_prop( $data['type'], 'label' ) ),
...
function acf_render_bidirectional_field_settings( $field ) {
	...
	acf_render_field_setting(
		$field,
		array(
			'label'        => __( 'Bidirectional', 'mytravel' ),
			'instructions' => __( 'Update a field on the selected values, referencing back to this ID', 'mytravel' ),
			'type'         => 'true_false',
			'name'         => 'bidirectional',
			'ui'           => 1,
		)
	...
	acf_render_field_setting(
		...
		array(
			...
			'label'        => __( 'Target Field', 'mytravel' ),
			'instructions' => __( 'Select field(s) to store the reference back to the item being updated. You may select this field. Target fields must be compatible with where this field is being displayed. For example, if this field is displayed on a Taxonomy, your target field should be of type Taxonomy', 'mytravel' ),
...
function acf_get_bidirectional_field_settings_instruction_text() {
	...
	$message = '<p class="acf-feature-notice with-warning-icon">' . sprintf( __( 'Enabling the bidirectional setting allows you to update a value in the target fields for each value selected for this field, adding or removing the Post ID, Taxonomy ID or User ID of the item being updated. For more information, please read the <a href="%s" target="_blank">documentation</a>.', 'mytravel' ), acf_add_url_utm_tags( 'https://www.advancedcustomfields.com/resources/bidirectional-relationships/', 'docs', 'bidirectional' ) ) . '</p>';
	return $message;
}
