...
if ( ! class_exists( 'ACF_Admin_Taxonomies' ) ) :
	...
	class ACF_Admin_Taxonomies extends ACF_Admin_Internal_Post_Type_List {
		...
		public function admin_menu() {
			...
			add_submenu_page( $parent_slug, __( 'Taxonomies', 'mytravel' ), __( 'Taxonomies', 'mytravel' ), $cap, 'edit.php?post_type=acf-taxonomy' );
		}
		...
		public function admin_table_columns( $_columns ) {
			...
			$columns = array(
				...
				'acf-post-types'   => __( 'Post Types', 'mytravel' ),
				'acf-field-groups' => __( 'Field Groups', 'mytravel' ),
				'acf-count'        => __( 'Terms', 'mytravel' ),
			);
		...
		public function render_admin_table_column_post_types( $taxonomy ) {
			...
			if ( empty( $labels ) ) {
				echo '<span class="acf-emdash" aria-hidden="true">—</span>';
				echo '<span class="screen-reader-text">' . esc_html__( 'No post types', 'mytravel' ) . '</span>';
				return;
			}
		...
		public function render_admin_table_column_num_terms( $taxonomy ) {
			...
			$no_terms .= '<span class="screen-reader-text">' . esc_html__( 'No terms', 'mytravel' ) . '</span>';
			...
			switch ( $action ) {
				case 'acfactivatecomplete':
					$text = sprintf(
						...
						_n( 'Taxonomy activated.', '%s taxonomies activated.', $count, 'mytravel' ),
						$count
					);
					break;
				case 'acfdeactivatecomplete':
					$text = sprintf(
						...
						_n( 'Taxonomy deactivated.', '%s taxonomies deactivated.', $count, 'mytravel' ),
						$count
					);
					break;
				case 'acfduplicatecomplete':
					$text = sprintf(
						...
						_n( 'Taxonomy duplicated.', '%s taxonomies duplicated.', $count, 'mytravel' ),
						$count
					);
					break;
				case 'acfsynccomplete':
					$text = sprintf(
						...
						_n( 'Taxonomy synchronized.', '%s taxonomies synchronized.', $count, 'mytravel' ),
						$count
					);
					break;
			}
