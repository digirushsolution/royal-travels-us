		...
		public function ajax_link_field_groups() {
			...
			if ( is_array( $args['field_groups'] ) && ! empty( $args['field_groups'] ) && $saved_post ) {
				...
				<div class="acf-actions">
					<button type="button" class="acf-btn acf-btn-secondary acf-close-popup"><?php esc_html_e( 'Close Modal', 'mytravel' ); ?></button>
				</div>
				<?php
			...
			$instructions = sprintf(
				...
				__( 'Add this %s to the location rules of the selected field groups.', 'mytravel' ),
				'acf-post-type' === $post_type ? __( 'post type', 'mytravel' ) : __( 'taxonomy', 'mytravel' )
			);
			...
			$field = acf_get_valid_field(
				array(
					...
					'choices'      => $choices,
					'aria-label'   => __( 'Please select the field groups to link.', 'mytravel' ),
					'placeholder'  => __( 'Select one or many field groups...', 'mytravel' ),
					'label'        => __( 'Field Group(s)', 'mytravel' ),
			...
			wp_send_json_success(
				array(
					'content' => $content,
					'title'   => esc_html__( 'Link Existing Field Groups', 'mytravel' ),
				)
			);
		}
	}
