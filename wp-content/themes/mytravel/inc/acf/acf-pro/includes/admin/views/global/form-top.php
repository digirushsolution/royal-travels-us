...
$acf_title_placeholder = apply_filters( 'enter_title_here', __( 'Add title','mytravel' ), $post );
...
if ( 'acf-field-group' === $acf_post_type ) {
	...
	$acf_prefilled_title = __( '%s fields', 'mytravel' );
	...
	if ( ! empty( $acf_duplicated_from_label ) ) {
		...
		$acf_duplicated_from = sprintf( __( ' (Duplicated from %s)', 'mytravel' ), $acf_duplicated_from_label );
	}
}
?>
	...
	<div class="acf-headerbar-inner">
		...
		<div class="acf-headerbar-content">
			...
			<div class="acf-title-wrap">
				...
				<input form="post" type="text" name="post_title" size="30" value="<?php echo esc_attr( $acf_title ); ?>" id="title" class="acf-headerbar-title-field" spellcheck="true" autocomplete="off" placeholder="<?php esc_attr_e( 'Field Group Title', 'mytravel' ); ?>" />
			</div>
			<?php endif; ?>
		</div>
