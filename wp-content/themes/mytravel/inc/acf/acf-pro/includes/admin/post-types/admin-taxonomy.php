	...
	class ACF_Admin_Taxonomy extends ACF_Admin_Internal_Post_Type {
		...
		public function post_updated_messages( $messages ) {
			$messages['acf-taxonomy'] = array(
				...
				2  => __( 'Taxonomy updated.', 'mytravel' ),
				3  => __( 'Taxonomy deleted.', 'mytravel' ),
				4  => $this->taxonomy_saved_message(),
				5  => false, // taxonomy does not support revisions.
				6  => $this->taxonomy_saved_message( true ),
				7  => __( 'Taxonomy saved.', 'mytravel' ),
				8  => __( 'Taxonomy submitted.', 'mytravel' ),
				9  => __( 'Taxonomy scheduled for.', 'mytravel' ),
				10 => __( 'Taxonomy draft updated.', 'mytravel' ),
			);
		...
		public function taxonomy_saved_message( $created = false ) {
			...
			$item_saved_text = sprintf( __( '%s taxonomy updated', 'mytravel' ), $title );
			/* translators: %s taxonomy name */
			$add_fields_text = sprintf( __( 'Add fields to %s', 'mytravel' ), $title );

			if ( $created ) {
				/* translators: %s taxonomy name */
				$item_saved_text = sprintf( __( '%s taxonomy created', 'mytravel' ), $title );
			}
			...
			<div class="acf-item-saved-links">
				...
				<a href="<?php echo esc_url( $duplicate_taxonomy_link ); ?>"><?php esc_html_e( 'Duplicate taxonomy', 'mytravel' ); ?></a>
		...
		public function admin_head() {
			...
			if ( ! empty( $acf_taxonomy['not_registered'] ) ) {
				acf_add_admin_notice(
					__( 'This taxonomy could not be registered because its key is in use by another taxonomy registered by another plugin or theme.', 'mytravel' ),
					'error'
				);
			}

			// metaboxes.
			add_meta_box( 'acf-basic-settings', __( 'Basic Settings', 'mytravel' ), array( $this, 'mb_basic_settings' ), 'acf-taxonomy', 'normal', 'high' );
			add_meta_box( 'acf-advanced-settings', __( 'Advanced Settings', 'mytravel' ), array( $this, 'mb_advanced_settings' ), 'acf-taxonomy', 'normal', 'high' );
		...
		public function post_submitbox_misc_actions() {
			global $acf_taxonomy;
			$status_label = $acf_taxonomy['active'] ? _x( 'Active', 'post status', 'mytravel' ) : _x( 'Inactive', 'post status', 'mytravel' );
