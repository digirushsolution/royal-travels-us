	...
	class acf_admin_field_group extends ACF_Admin_Internal_Post_Type {
		...
		public function post_updated_messages( $messages ) {
			$messages['acf-field-group'] = array(
				...
				1  => __( 'Field group updated.', 'mytravel' ),
				2  => __( 'Field group updated.', 'mytravel' ),
				3  => __( 'Field group deleted.', 'mytravel' ),
				4  => __( 'Field group updated.', 'mytravel' ),
				5  => false, // field group does not support revisions.
				6  => __( 'Field group published.', 'mytravel' ),
				7  => __( 'Field group saved.', 'mytravel' ),
				8  => __( 'Field group submitted.', 'mytravel' ),
				9  => __( 'Field group scheduled for.', 'mytravel' ),
				10 => __( 'Field group draft updated.', 'mytravel' ),
			);
		...
		public function admin_enqueue_scripts() {
			...
			acf_localize_text(
				array(
					'The string "field_" may not be used at the start of a field name' => __( 'The string "field_" may not be used at the start of a field name', 'mytravel' ),
					'This field cannot be moved until its changes have been saved' => __( 'This field cannot be moved until its changes have been saved', 'mytravel' ),
					'Field group title is required'     => __( 'Field group title is required', 'mytravel' ),
					'Move field group to trash?'        => __( 'Move field group to trash?', 'mytravel' ),
					'No toggle fields available'        => __( 'No toggle fields available', 'mytravel' ),
					'Move Custom Field'                 => __( 'Move Custom Field', 'mytravel' ),
					'Close modal'                       => __( 'Close modal', 'mytravel' ),
					'Field moved to other group'        => __( 'Field moved to other group', 'mytravel' ),
					'Field groups linked successfully.' => __( 'Field groups linked successfully.', 'mytravel' ),
					'Checked'                           => __( 'Checked', 'mytravel' ),
					'(no label)'                        => __( '(no label)', 'mytravel' ),
					'(this field)'                      => __( '(this field)', 'mytravel' ),
					'copy'                              => __( 'copy', 'mytravel' ),
					'or'                                => __( 'or', 'mytravel' ),
					'Show this field group if'          => __( 'Show this field group if', 'mytravel' ),
					'Null'                              => __( 'Null', 'mytravel' ),
					'PRO Only'                          => __( 'PRO Only', 'mytravel' ),

					// Conditions.
					'Has any value'                     => __( 'Has any value', 'mytravel' ),
					'Has no value'                      => __( 'Has no value', 'mytravel' ),
					'Value is equal to'                 => __( 'Value is equal to', 'mytravel' ),
					'Value is not equal to'             => __( 'Value is not equal to', 'mytravel' ),
					'Value matches pattern'             => __( 'Value matches pattern', 'mytravel' ),
					'Value contains'                    => __( 'Value contains', 'mytravel' ),
					'Value is greater than'             => __( 'Value is greater than', 'mytravel' ),
					'Value is less than'                => __( 'Value is less than', 'mytravel' ),
					'Selection is greater than'         => __( 'Selection is greater than', 'mytravel' ),
					'Selection is less than'            => __( 'Selection is less than', 'mytravel' ),

					// Custom Select2 templates.
					'Type to search...'                 => __( 'Type to search...', 'mytravel' ),
					'This Field'                        => __( 'This Field', 'mytravel' ),
				)
			);
			...
			acf_localize_data(
				array(
					...
					'PROLocationTypes'    => array(
						'block'        => __( 'Block', 'mytravel' ),
						'options_page' => __( 'Options Page', 'mytravel' ),
					),
				)
			);
		...
		public function admin_head() {
			...
			// metaboxes.
			add_meta_box( 'acf-field-group-fields', __( 'Fields', 'mytravel' ), array( $this, 'mb_fields' ), 'acf-field-group', 'normal', 'high' );
			add_meta_box( 'acf-field-group-options', __( 'Settings', 'mytravel' ), array( $this, 'mb_options' ), 'acf-field-group', 'normal', 'high' );
		...
		public function screen_settings( $html ) {
			...
			$html .= '<label for="acf-field-key-hide"><input id="acf-field-key-hide" type="checkbox" value="1" name="show_field_keys" ' . $show_field_keys . ' /> ' . __( 'Field Keys', 'mytravel' ) . '</label>';

			if ( ! $hide_field_settings_tabs ) {
				$html .= '<label for="acf-field-settings-tabs"><input id="acf-field-settings-tabs" type="checkbox" value="1" name="show_field_settings_tabs" ' . $show_field_settings_tabs . ' />' . __( 'Field Settings Tabs', 'mytravel' ) . '</label>';
			}
		...
		public function post_submitbox_misc_actions() {
			global $field_group;
			$status_label = $field_group['active'] ? _x( 'Active', 'post status', 'mytravel' ) : _x( 'Inactive', 'post status', 'mytravel' );
		...
		public function ajax_move_field() {
			...
			if ( $args['field_id'] && $args['field_group_id'] ) {
				...
				echo '' .
					'<p><strong>' . __( 'Move Complete.', 'mytravel' ) . '</strong></p>' .
					'<p>' . sprintf(
						/* translators: Confirmation message once a field has been moved to a different field group. */
						acf_punctify( __( 'The %1$s field can now be found in the %2$s field group', 'mytravel' ) ),
						esc_html( $field['label'] ),
						$link  //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					) . '</p>' .
					'<a href="#" class="button button-primary acf-close-popup">' . __( 'Close Modal', 'mytravel' ) . '</a>';
				die();
			}
			...
			$field = acf_get_valid_field(
				array(
					...
					'choices'    => $choices,
					'aria-label' => __( 'Please select the destination for this field', 'mytravel' ),
				)
			);

			echo '<p>' . __( 'Please select the destination for this field', 'mytravel' ) . '</p>';
			...
			echo '<form id="acf-move-field-form">';
				...
				echo '<button type="submit" class="acf-btn">' . __( 'Move Field', 'mytravel' ) . '</button>';
