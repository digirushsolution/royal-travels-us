...
if ( ! class_exists( 'ACF_Admin_Tool_Export' ) ) :
	...
	class ACF_Admin_Tool_Export extends ACF_Admin_Tool {
		...
		function initialize() {
			...
			// vars
			$this->name  = 'export';
			$this->title = __( 'Export Field Groups', 'mytravel' );

			// active
			if ( $this->is_active() ) {
				$this->title .= ' - ' . __( 'Generate PHP', 'mytravel' );
			}
		}
		...
		function submit_download() {
			...
			// validate
			if ( $json === false ) {
				return acf_add_admin_notice( __( 'No field groups selected', 'mytravel' ), 'warning' );
			}
		...
		function load() {
			...
			if ( $this->is_active() ) {
				...
				if ( $selected ) {
					...
					$text  = sprintf( _n( 'Exported 1 item.', 'Exported %s items.', $count, 'mytravel' ), $count );
		...
		public function html_field_selection() {
			...
			acf_render_field_wrap(
				array(
					'label'   => __( 'Select Field Groups', 'mytravel' ),
					'type'    => 'checkbox',
					'name'    => 'keys',
					'prefix'  => false,
					'value'   => $selected,
			...
			if ( $post_types ) {
				...
				acf_render_field_wrap(
					array(
						'label'   => __( 'Select Post Types', 'mytravel' ),
						'type'    => 'checkbox',
						'name'    => 'post_type_keys',
						'prefix'  => false,
						'value'   => $selected,
			...
			if ( $taxonomies ) {
				...
				acf_render_field_wrap(
					array(
						'label'   => __( 'Select Taxonomies', 'mytravel' ),
						'type'    => 'checkbox',
						'name'    => 'taxonomy_keys',
						'prefix'  => false,
						'value'   => $selected,
			...
			if ( $options_pages ) {
				...
				acf_render_field_wrap(
					array(
						'label'   => __( 'Select Options Pages', 'mytravel' ),
						'type'    => 'checkbox',
		...
		<div class="acf-panel acf-panel-settings">
			<h3 class="acf-panel-title"><?php _e( 'Settings', 'mytravel' ); ?> <i class="dashicons dashicons-arrow-right"></i></h3>
			<div class="acf-panel-inside">
				<?php

				/*
				acf_render_field_wrap(array(
					'label'     => __('Empty settings', 'mytravel'),
					'type'      => 'select',
					'name'      => 'minimal',
					'prefix'    => false,
					'value'     => '',
					'choices'   => array(
						'all'       => __('Include all settings', 'mytravel'),
						'minimal'   => __('Ignore empty settings', 'mytravel'),
					)
				));
				*/
		...
		<div class="acf-postbox-inner">
			...
			<p class="acf-submit acf-actions-strip">
				<button type="submit" name="action" class="acf-btn acf-button-primary" value="download"><?php _e( 'Export As JSON', 'mytravel' ); ?></button>
				<button type="submit" name="action" class="acf-btn acf-btn-secondary" value="generate"><?php _e( 'Generate PHP', 'mytravel' ); ?></button>
			</p>
		</div>
			<?php
		}
		...
		public function html_single() {
			?>
			<div class="acf-postbox-header">
				<h2 class="acf-postbox-title"><?php esc_html_e( 'Export - Generate PHP', 'mytravel' ); ?></h2>
				<i tabindex="0" class="acf-icon acf-icon-help acf-js-tooltip" title="<?php esc_attr_e( "The following code can be used to register a local version of the selected items. Storing field groups, post types, or taxonomies locally can provide many benefits such as faster load times, version control & dynamic fields/settings. Simply copy and paste the following code to your theme's functions.php file or include it within an external file, then deactivate or delete the items from the ACF admin.", 'mytravel' ); ?>">?</i>
			</div>
			<div class="acf-postbox-columns">
				...
				<div class="acf-postbox-side">
					...
					<p class="acf-submit">
						<button type="submit" name="action" class="acf-btn" value="generate"><?php esc_html_e( 'Generate PHP', 'mytravel' ); ?></button>
					</p>
				</div>
			</div>
			<?php
		...
		public function html_generate() {
			...
			?>
			<p class="acf-submit">
				<a class="button" id="acf-export-copy"><?php _e( 'Copy to clipboard', 'mytravel' ); ?></a>
			</p>
			...
			(function($){
				...
				$a.on('click', function( e ){
					...
					try {
						...
						acf.newTooltip({
							text: 		"<?php esc_html_e( 'Copied', 'mytravel' ); ?>",
							timeout:	250,
							target: 	$(this),
						});
					} catch (err) {
