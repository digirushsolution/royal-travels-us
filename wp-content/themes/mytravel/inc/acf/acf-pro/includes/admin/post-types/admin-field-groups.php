...
if ( ! class_exists( 'ACF_Admin_Field_Groups' ) ) :
	...
	class ACF_Admin_Field_Groups extends ACF_Admin_Internal_Post_Type_List {
		...
		public function admin_menu() {
			...
			add_submenu_page( $parent_slug, __( 'Field Groups', 'mytravel' ), __( 'Field Groups', 'mytravel' ), $cap, $parent_slug );
		}
		...
		public function admin_table_columns( $_columns ) {
			...
			$columns = array(
				...
				'acf-location'    => __( 'Location', 'mytravel' ),
				'acf-count'       => __( 'Fields', 'mytravel' ),
			);
		...
		public function render_admin_table_column_locations( $field_group ) {
			...
			if ( $objects ) {
				...
				$html = '<span class="dashicons dashicons-businesswoman"></span> ' . __( 'Various', 'mytravel' );
			}
		...
		public function render_admin_table_column_num_fields( $field_group ) {
			...
			if ( ! $field_count || ! is_numeric( $field_count ) ) {
				echo '<span class="acf-emdash" aria-hidden="true">—</span>';
				echo '<span class="screen-reader-text">' . esc_html__( 'No fields', 'mytravel' ) . '</span>';
				return;
			}
			...
			switch ( $action ) {
				case 'acfactivatecomplete':
					$text = sprintf(
						...
						_n( 'Field group activated.', '%s field groups activated.', $count, 'mytravel' ),
						$count
					);
					break;
				case 'acfdeactivatecomplete':
					$text = sprintf(
						...
						_n( 'Field group deactivated.', '%s field groups deactivated.', $count, 'mytravel' ),
						$count
					);
					break;
				case 'acfduplicatecomplete':
					$text = sprintf(
						...
						_n( 'Field group duplicated.', '%s field groups duplicated.', $count, 'mytravel' ),
						$count
					);
					break;
				case 'acfsynccomplete':
					$text = sprintf(
						...
						_n( 'Field group synchronized.', '%s field groups synchronized.', $count, 'mytravel' ),
						$count
					);
					break;
			}
