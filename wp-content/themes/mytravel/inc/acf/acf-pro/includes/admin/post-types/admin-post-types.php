...
if ( ! class_exists( 'ACF_Admin_Post_Types' ) ) :
	...
	class ACF_Admin_Post_Types extends ACF_Admin_Internal_Post_Type_List {
		...
		public function admin_menu() {
			...
			add_submenu_page( $parent_slug, __( 'Post Types', 'mytravel' ), __( 'Post Types', 'mytravel' ), $cap, 'edit.php?post_type=acf-post-type' );
		}
		...
		public function admin_table_columns( $_columns ) {
			...
			$columns = array(
				...
				'acf-description'  => __( 'Description', 'mytravel' ),
				'acf-key'          => __( 'Key', 'mytravel' ),
				'acf-taxonomies'   => __( 'Taxonomies', 'mytravel' ),
				'acf-field-groups' => __( 'Field Groups', 'mytravel' ),
				'acf-count'        => __( 'Posts', 'mytravel' ),
			);

			if ( acf_get_local_json_files( $this->post_type ) ) {
				$columns['acf-json'] = __( 'Local JSON', 'mytravel' );
			}
			...
			switch ( $column_name ) {
				...
				case 'acf-description':
					if ( is_string( $post['description'] ) && ! empty( $post['description'] ) ) {
						...
						echo '<span class="screen-reader-text">' . esc_html__( 'No description', 'mytravel' ) . '</span>';
					}
					break;
		...
		public function render_admin_table_column_field_groups( $post_type ) {
			...
			if ( empty( $field_groups ) ) {
				echo '<span class="acf-emdash" aria-hidden="true">—</span>';
				echo '<span class="screen-reader-text">' . esc_html__( 'No field groups', 'mytravel' ) . '</span>';
				return;
			}
		...
		public function render_admin_table_column_taxonomies( $post_type ) {
			...
			if ( empty( $labels ) ) {
				echo '<span class="acf-emdash" aria-hidden="true">—</span>';
				echo '<span class="screen-reader-text">' . esc_html__( 'No taxonomies', 'mytravel' ) . '</span>';
				return;
			}
		...
		public function render_admin_table_column_num_posts( $post_type ) {
			...
			$no_posts .= '<span class="screen-reader-text">' . esc_html__( 'No posts', 'mytravel' ) . '</span>';
			...
			switch ( $action ) {
				case 'acfactivatecomplete':
					$text = sprintf(
						...
						_n( 'Post type activated.', '%s post types activated.', $count, 'mytravel' ),
						$count
					);
					break;
				case 'acfdeactivatecomplete':
					$text = sprintf(
						...
						_n( 'Post type deactivated.', '%s post types deactivated.', $count, 'mytravel' ),
						$count
					);
					break;
				case 'acfduplicatecomplete':
					$text = sprintf(
						...
						_n( 'Post type duplicated.', '%s post types duplicated.', $count, 'mytravel' ),
						$count
					);
					break;
				case 'acfsynccomplete':
					$text = sprintf(
						...
						_n( 'Post type synchronized.', '%s post types synchronized.', $count, 'mytravel' ),
						$count
					);
					break;
			}
