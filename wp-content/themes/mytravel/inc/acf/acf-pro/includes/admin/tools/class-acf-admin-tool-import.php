...
if ( ! class_exists( 'ACF_Admin_Tool_Import' ) ) :
	...
	class ACF_Admin_Tool_Import extends ACF_Admin_Tool {
		...
		function initialize() {
			...
			// vars
			$this->name  = 'import';
			$this->title = __( 'Import Field Groups', 'mytravel' );
		...
		<div class="acf-postbox-header">
			<h2 class="acf-postbox-title"><?php esc_html_e( 'Import', 'mytravel' ); ?></h2>
			<div class="acf-tip"><i tabindex="0" class="acf-icon acf-icon-help acf-js-tooltip" title="<?php esc_attr_e( 'Select the Advanced Custom Fields JSON file you would like to import. When you click the import button below, ACF will import the items in that file.', 'mytravel' ); ?>">?</i></div>
		</div>
		<div class="acf-postbox-inner">
			...
			<p class="acf-submit">
				<button type="submit" class="acf-btn" name="import_type" value="json">
					<?php esc_html_e( 'Import JSON', 'mytravel' ); ?>
				</button>
			</p>
			...
			if ( is_plugin_active( 'custom-post-type-ui/custom-post-type-ui.php' ) && acf_get_setting( 'enable_post_types' ) ) {
				...
				if ( $cptui_post_types ) {
					$choices['post_types'] = __( 'Post Types', 'mytravel' );
				...
				if ( $cptui_taxonomies ) {
					$choices['taxonomies'] = __( 'Taxonomies', 'mytravel' );
				...
				if ( ! empty( $choices ) ) :
					...
					<div class="acf-fields import-cptui">
						...
						acf_render_field_wrap(
							array(
								'label'   => __( 'Import from Custom Post Type UI', 'mytravel' ),
								'type'    => 'checkbox',
					...
					<?php
					if ( $overwrite_warning ) {
						echo '<p class="acf-inline-notice notice notice-info">' . esc_html__( 'Importing a Post Type or Taxonomy with the same key as one that already exists will overwrite the settings for the existing Post Type or Taxonomy with those of the import.', 'mytravel' ) . '</p>';
					}
					?>
					<p class="acf-submit">
						<button type="submit" class="acf-btn" name="import_type" value="cptui">
							<?php esc_html_e( 'Import from Custom Post Type UI', 'mytravel' ); ?>
						</button>
					</p>
					<?php
				endif;
		...
		public function submit() {
			...
			if ( empty( $_FILES['acf_import_file']['size'] ) ) {
				return acf_add_admin_notice( __( 'No file selected', 'mytravel' ), 'warning' );
			}
			...
			// Check errors.
			if ( $file['error'] ) {
				return acf_add_admin_notice( __( 'Error uploading file. Please try again', 'mytravel' ), 'warning' );
			}

			// Check file type.
			if ( pathinfo( $file['name'], PATHINFO_EXTENSION ) !== 'json' ) {
				return acf_add_admin_notice( __( 'Incorrect file type', 'mytravel' ), 'warning' );
			}
			...
			// Generate text.
			$text = sprintf( _n( 'Imported 1 item', 'Imported %s items', $total, 'mytravel' ), $total );
		...
		public function import_cpt_ui( $import_args ) {
			if ( ! is_array( $import_args ) ) {
				return acf_add_admin_notice( __( 'Nothing from Custom Post Type UI plugin selected for import.', 'mytravel' ), 'warning' );
			}
			...
			if ( in_array( 'post_types', $import_args, true ) ) {
				...
				if ( ! is_array( $cptui_post_types ) || ! $instance ) {
					return acf_add_admin_notice( __( 'Failed to import post types.', 'mytravel' ), 'warning' );
				}
			...
			if ( in_array( 'taxonomies', $import_args, true ) ) {
				...
				if ( ! is_array( $cptui_taxonomies ) || ! $instance ) {
					return acf_add_admin_notice( __( 'Failed to import taxonomies.', 'mytravel' ), 'warning' );
				}
			...
			if ( ! empty( $imported ) ) {
				...
				$text = sprintf( _n( 'Imported %d item from Custom Post Type UI -', 'Imported %d items from Custom Post Type UI -', $total, 'mytravel' ), $total );
				...
				$text .= ' ' . implode( ', ', $links );
				$text .= __( '. The Custom Post Type UI plugin can be deactivated.', 'mytravel' );

				return acf_add_admin_notice( $text, 'success' );
			}

			return acf_add_admin_notice( __( 'Nothing to import', 'mytravel' ), 'warning' );
		}
	}
