...
if ( ! class_exists( 'ACF_Admin_Internal_Post_Type_List' ) ) :
	...
	class ACF_Admin_Internal_Post_Type_List {
		...
		public function current_screen() {
			...
			$wp_post_statuses['publish']->label_count = _n_noop( 'Active <span class="count">(%s)</span>', 'Active <span class="count">(%s)</span>', 'mytravel' );
		...
		public function admin_enqueue_scripts() {
			...
			acf_localize_text(
				array(
					'Review local JSON changes' => __( 'Review local JSON changes', 'mytravel' ),
					'Loading diff'              => __( 'Loading diff', 'mytravel' ),
					'Sync changes'              => __( 'Sync changes', 'mytravel' ),
				)
			);
		}
		...
		public function get_disabled_post_state() {
			return '<span class="dashicons dashicons-hidden"></span> ' . _x( 'Inactive', 'post status', 'mytravel' );
		}
		...
		public function get_registration_error_state() {
			...
			__( 'This item could not be registered because its key is in use by another item registered by another plugin or theme.', 'mytravel' ) .
			'"></span> ' . _x( 'Registration Failed', 'post status', 'mytravel' );
		}
		...
		public function get_human_readable_file_location( $file ) {
			...
			if ( strpos( $file, $theme_path ) !== false ) {
				...
				$located  = sprintf( __( 'Located in theme: %s', 'mytravel' ), $rel_file );
			} elseif ( strpos( $file, WP_PLUGIN_DIR ) !== false ) {
				$rel_file = str_replace( WP_PLUGIN_DIR, '', $file );
				$located  = sprintf( __( 'Located in plugin: %s', 'mytravel' ), $rel_file );
			} else {
				$rel_file = str_replace( ABSPATH, '', $file );
				$located  = sprintf( __( 'Located in: %s', 'mytravel' ), $rel_file );
			}
		...
		public function render_admin_table_column_local_status( $post ) {
			...
			if ( isset( $json[ $post['key'] ] ) ) {
				if ( isset( $this->sync[ $post['key'] ] ) ) {
					...
					echo '<strong>' . __( 'Sync available', 'mytravel' ) . '</strong>';
					if ( $post['ID'] ) {
						echo '<div class="row-actions">
                            <span class="sync"><a href="' . esc_url( $url ) . '">' . __( 'Sync', 'mytravel' ) . '</a> | </span>
                            <span class="review"><a href="#" data-event="review-sync" data-id="' . esc_attr( $post['ID'] ) . '" data-href="' . esc_url( $url ) . '">' . __( 'Review changes', 'mytravel' ) . '</a></span>
                        </div>';
					} else {
						echo '<div class="row-actions">
                            <span class="sync"><a href="' . esc_url( $url ) . '">' . __( 'Import', 'mytravel' ) . '</a></span>
                        </div>';
					}
				} else {
					echo __( 'Saved', 'mytravel' );
				}
			} else {
				echo '<span class="acf-secondary-text">' . __( 'Awaiting save', 'mytravel' ) . '</span>';
			}
		}
		...
		public function page_row_actions( $actions, $post ) {
			...
			$actions['acfduplicate'] = '<a href="' . esc_url( $duplicate_action_url ) . '" aria-label="' . esc_attr__( 'Duplicate this item', 'mytravel' ) . '">' . __( 'Duplicate', 'mytravel' ) . '</a>';
			...
			if ( 'acf-disabled' === $post->post_status ) {
				...
				$actions['acfactivate']     = '<a href="' . esc_url( $activate_action_url ) . '" aria-label="' . esc_attr__( 'Activate this item', 'mytravel' ) . '">' . __( 'Activate', 'mytravel' ) . '</a>';
			} else {
				...
				$actions['acfdeactivate']   = '<a href="' . esc_url( $deactivate_action_url ) . '" aria-label="' . esc_attr__( 'Deactivate this item', 'mytravel' ) . '">' . __( 'Deactivate', 'mytravel' ) . '</a>';
			}
		...
		public function admin_table_bulk_actions( $actions ) {
			if ( ! in_array( $this->view, array( 'sync', 'trash' ), true ) ) {
				...
				if ( 'acf-field-group' === $this->post_type ) {
					$actions['acfduplicate'] = __( 'Duplicate', 'mytravel' );
				}

				$actions['acfactivate']   = __( 'Activate', 'mytravel' );
				$actions['acfdeactivate'] = __( 'Deactivate', 'mytravel' );
			}
			...
			if ( $this->sync ) {
				...
				$actions['acfsync'] = __( 'Sync changes', 'mytravel' );
			}
		...
		public function admin_table_views( $views ) {
			...
			if ( $count ) {
				$views['sync'] = sprintf(
					...
					esc_html( __( 'Sync available', 'mytravel' ) ),
					$count
				);
			}
					...
					foreach ( $this->sync as $k => $field_group ) {
						...
						foreach ( $columns as $column_name => $column_label ) {
							...
							switch ( $column_name ) {
								...
								// Checkbox.
								case 'cb':
									echo '<label for="cb-select-' . esc_attr( $k ) . '" class="screen-reader-text">' . esc_html( sprintf( __( 'Select %s', 'mytravel' ), $field_group['title'] ) ) . '</label>';
