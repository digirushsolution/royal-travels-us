...
if ( isset( $field['conditional_logic'] ) && is_array( $field['conditional_logic'] ) && count( $field['conditional_logic'] ) > 0 ) {
	...
	$conditional_logic_text  = __( 'Active', 'mytravel' );
}
	...
	<div class="handle">
		<ul class="acf-hl acf-tbody">
			<li class="li-field-order">
				<span class="acf-icon acf-sortable-handle" title="<?php _e( 'Drag to reorder', 'mytravel' ); ?>"><?php echo ( $i + 1 ); ?></span>
			</li>
			<li class="li-field-label">
				<strong>
					<a class="edit-field" title="<?php _e( 'Edit field', 'mytravel' ); ?>" href="#"><?php echo acf_esc_html( $field_label ); ?></a>
				</strong>
				<div class="row-options">
					<a class="edit-field" title="<?php _e( 'Edit field', 'mytravel' ); ?>" href="#"><?php _e( 'Edit', 'mytravel' ); ?></a>
					<a class="duplicate-field" title="<?php _e( 'Duplicate field', 'mytravel' ); ?>" href="#"><?php _e( 'Duplicate', 'mytravel' ); ?></a>
					<?php if ( $num_field_groups > 1 ) : ?>
					<a class="move-field" title="<?php _e( 'Move field to another group', 'mytravel' ); ?>" href="#"><?php _e( 'Move', 'mytravel' ); ?></a>
					<?php endif; ?>
					<a class="delete-field" title="<?php _e( 'Delete field', 'mytravel' ); ?>" href="#"><?php _e( 'Delete', 'mytravel' ); ?></a>
				</div>
			</li>
			...
			<div class="acf-field-settings acf-fields">
				...
				foreach ( acf_get_combined_field_type_settings_tabs() as $tab_key => $tab_label ) {
					...
					if ( $tab_key === 'conditional_logic' ) {
						$field_to_render['label'] = __( 'Conditional Logic', 'mytravel' ) . '<i class="conditional-logic-badge' . $conditional_logic_class . '">' . $conditional_logic_text . '</i>';
					}
						...
						switch ( $tab_key ) {
							case 'general':
								...
								acf_render_field_setting(
									$field,
									array(
										'label'        => __( 'Field Label', 'mytravel' ),
										'instructions' => __( 'This is the name which will appear on the EDIT page', 'mytravel' ),
										'name'         => 'label',
										'type'         => 'text',
										'class'        => 'field-label',
									),
							...
							case 'presentation':
								acf_render_field_setting(
									$field,
									array(
										'label'        => __( 'Instructions', 'mytravel' ),
										'instructions' => __( 'Instructions for authors. Shown when submitting data', 'mytravel' ),
										'type'         => 'textarea',
										'name'         => 'instructions',
										'rows'         => 5,
									),
								...
								acf_render_field_wrap(
									array(
										'label'        => __( 'Wrapper Attributes', 'mytravel' ),
										'instructions' => '',
										'type'         => 'number',
										'name'         => 'width',
										'prefix'       => $field['prefix'] . '[wrapper]',
				...
				<div class="acf-field-settings-footer">
					<a class="button close-field edit-field" title="<?php _e( 'Close Field', 'mytravel' ); ?>" href="#"><?php _e( 'Close Field', 'mytravel' ); ?></a>
				</div>
			</div>
		</div>
	</div>
