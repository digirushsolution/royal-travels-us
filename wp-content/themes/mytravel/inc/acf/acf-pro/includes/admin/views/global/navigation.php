...
if ( ! acf_get_setting( 'pro' ) ) {
	$acf_more_items[] = array(
		...
		'text'   => __( 'Options Pages', 'mytravel' ) . '<span class="acf-requires-pro">' . __( 'PRO', 'mytravel' ) . '</span>',
		'target' => '_self',
	);
}
...
if ( ! defined( 'PWP_NAME' ) ) {
	...
	$wpengine_more_items[] = array(
		...
		'text'     => $acf_wpengine_logo . '<span class="acf-wp-engine-upsell-pill">' . __( '4 Months Free', 'mytravel' ) . '</span>',
		'target'   => '_blank',
	...
	<div class="acf-admin-toolbar-inner">
		<div class="acf-nav-wrap">
			<a href="<?php echo admin_url( 'edit.php?post_type=acf-field-group' ); ?>" class="acf-logo">
				<img src="<?php echo acf_get_url( 'assets/images/acf-logo.svg' ); ?>" alt="<?php esc_attr_e( 'Advanced Custom Fields logo', 'mytravel' ); ?>">
			...
			<?php if ( $acf_more_items || $more_items ) { ?>
				<div class="acf-more acf-header-tab-acf-more" tabindex="0">
					<span class="acf-tab acf-more-tab"><i class="acf-icon acf-icon-more"></i><?php esc_html_e( 'More', 'mytravel' ); ?> <i class="acf-icon acf-icon-dropdown"></i></span>
					<ul>
						<?php
						if ( $acf_more_items ) {
							if ( $more_items ) {
							...
							acf_print_menu_section( $acf_more_items, 'mytravel' );
						}
						if ( $more_items ) {
							echo '<li class="acf-more-section-header"><span class="acf-tab acf-tab-header">' . esc_html__( 'Other', 'mytravel' ) . ' </span></li>';
		...
		<div class="acf-nav-upgrade-wrap">
			...
			if ( ! acf_is_pro() || ! acf_pro_is_license_active() ) {
				...
				$unlock_text   = __( 'Unlock Extra Features with ACF PRO', 'mytravel' );
				...
				if ( acf_is_pro() ) {
					...
					if ( acf_pro_is_license_expired() ) {
						...
						$unlock_target = '_blank';
						$unlock_text   = __( 'Renew ACF PRO License', 'mytravel' );
					}
				}
				?>
			...
			<a href="<?php echo $acf_wpengine_logo_link; ?>" target="_blank" class="acf-nav-wpengine-logo">
				<img src="<?php echo esc_url( acf_get_url( 'assets/images/wp-engine-horizontal-white.svg' ) ); ?>" alt="<?php esc_html_e( 'WP Engine logo', 'mytravel' ); ?>" />
			</a>
		</div>
	</div>
</div>
...
if ( ! in_array( $screen->id, acf_get_internal_post_types(), true ) ) {
	if ( $plugin_page == 'acf-tools' ) {
		$acf_page_title = __( 'Tools', 'mytravel' );
	} elseif ( $plugin_page == 'acf-settings-updates' ) {
		$acf_page_title = __( 'Updates', 'mytravel' );
	} elseif ( $plugin_page == 'acf_options_preview' && ! acf_is_pro() ) {
		$acf_page_title = __( 'Options Pages', 'mytravel' );
	}
