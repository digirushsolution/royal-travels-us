	...
	class ACF_Admin_Post_type extends ACF_Admin_Internal_Post_Type {
		...
		public function post_updated_messages( $messages ) {
			$messages['acf-post-type'] = array(
				...
				3  => __( 'Post type deleted.', 'mytravel' ),
				4  => __( 'Post type updated.', 'mytravel' ),
				5  => false, // Post type does not support revisions.
				6  => $this->post_type_created_message( true ), // Created.
				7  => __( 'Post type saved.', 'mytravel' ),
				8  => __( 'Post type submitted.', 'mytravel' ),
				9  => __( 'Post type scheduled for.', 'mytravel' ),
				10 => __( 'Post type draft updated.', 'mytravel' ),
			);
		...
		public function post_type_created_message( $created = false ) {
			...
			$item_saved_text = sprintf( __( '%s post type updated', 'mytravel' ), $title );

			if ( $created ) {
				/* translators: %s post type name */
				$item_saved_text = sprintf( __( '%s post type created', 'mytravel' ), $title );
			}
			...
			<div class="acf-item-saved-links">
				<a href="<?php echo esc_url( $add_fields_link ); ?>"><?php esc_html_e( 'Add fields', 'mytravel' ); ?></a>
				<a class="acf-link-field-groups" href="#"><?php esc_html_e( 'Link field groups', 'mytravel' ); ?></a>
				<a href="<?php echo esc_url( $create_post_type_link ); ?>"><?php esc_html_e( 'Create post type', 'mytravel' ); ?></a>
				<a href="<?php echo esc_url( $duplicate_post_type_link ); ?>"><?php esc_html_e( 'Duplicate post type', 'mytravel' ); ?></a>
				<a href="<?php echo esc_url( $create_taxonomy_link ); ?>"><?php esc_html_e( 'Create taxonomy', 'mytravel' ); ?></a>
			</div>
			<?php
			return ob_get_clean();
		}
		...
		public function admin_enqueue_scripts() {
			...
			acf_localize_text(
				array(
					'Post'    => __( 'Post', 'mytravel' ),
					'Posts'   => __( 'Posts', 'mytravel' ),
					'Page'    => __( 'Page', 'mytravel' ),
					'Pages'   => __( 'Pages', 'mytravel' ),
					'Default' => __( 'Default', 'mytravel' ),
				)
			);
		...
		public function admin_head() {
			...
			if ( ! empty( $acf_post_type['not_registered'] ) ) {
				acf_add_admin_notice(
					__( 'This post type could not be registered because its key is in use by another post type registered by another plugin or theme.', 'mytravel' ),
					'error'
				);
			}

			// metaboxes.
			add_meta_box( 'acf-basic-settings', __( 'Basic Settings', 'mytravel' ), array( $this, 'mb_basic_settings' ), 'acf-post-type', 'normal', 'high' );
			add_meta_box( 'acf-advanced-settings', __( 'Advanced Settings', 'mytravel' ), array( $this, 'mb_advanced_settings' ), 'acf-post-type', 'normal', 'high' );
		...
		public function post_submitbox_misc_actions() {
			...
			$status_label = $acf_post_type['active'] ? _x( 'Active', 'post status', 'mytravel' ) : _x( 'Inactive', 'post status', 'mytravel' );
