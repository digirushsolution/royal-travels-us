...
if ( ! class_exists( 'acf_field_post_object' ) ) :
	...
	class acf_field_post_object extends acf_field {
		...
		public function initialize() {
			...
			$this->label         = __( 'Post Object', 'mytravel' );
			$this->category      = 'relational';
			$this->description   = __( 'An interactive and customizable UI for picking one or many posts, pages or post type items with the option to search. ', 'mytravel' );
		...
		*/
		...
		public function validate_rest_value( $valid, $value, $field ) {
			...
			if ( count( $invalid_posts ) ) {
				$error         = sprintf(
					__( '%1$s must have a valid post ID.', 'mytravel' ),
					$param
				);
