	...
	class acf_field_color_picker extends acf_field {
		...
		*/
		...
		function initialize() {
			...
			$this->label         = __( 'Color Picker', 'mytravel' );
			$this->category      = 'advanced';
			$this->description   = __( 'An interactive UI for selecting a color, or specifying a Hex value.', 'mytravel' );
			...
			if ( ! is_admin() ) {
				...
				if ( method_exists( $scripts, 'set_translations' ) ) {
					...
					$scripts->localize(
						...
						'wpColorPickerL10n',
						array(
							'clear'            => __( 'Clear', 'mytravel' ),
							'clearAriaLabel'   => __( 'Clear color', 'mytravel' ),
							'defaultString'    => __( 'Default', 'mytravel' ),
							'defaultAriaLabel' => __( 'Select default color', 'mytravel' ),
							'pick'             => __( 'Select Color', 'mytravel' ),
							'defaultLabel'     => __( 'Color value', 'mytravel' ),
						)
					);
				}
			}
			...
			acf_localize_data(
				array(
					'colorPickerL10n' => array(
						'hex_string'  => __( 'Hex String', 'mytravel' ),
						'rgba_string' => __( 'RGBA String', 'mytravel' ),
					),
				)
			);
		}
		...
		*/
		...
		function render_field_settings( $field ) {
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Enable Transparency', 'mytravel' ),
					'instructions' => '',
					'type'         => 'true_false',
					'name'         => 'enable_opacity',
					'ui'           => 1,
			...
			acf_render_field_setting(
				...
				array(
					...
					'layout'       => 'horizontal',
					'choices'      => array(
						'string' => __( 'Hex String', 'mytravel' ),
						'array'  => __( 'RGBA Array', 'mytravel' ),
					),
				)
			);
		}
