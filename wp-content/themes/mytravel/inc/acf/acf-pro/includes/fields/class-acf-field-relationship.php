...
if ( ! class_exists( 'acf_field_relationship' ) ) :
	...
	class acf_field_relationship extends acf_field {
		...
		public function initialize() {
			...
			$this->label         = __( 'Relationship', 'mytravel' );
			$this->category      = 'relational';
			$this->description   = __( 'A dual-column interface to select one or more posts, pages, or custom post type items to create a relationship with the item that you\'re currently editing. Includes options to search and filter.', 'mytravel' );
		...
		*/
		...
		function input_admin_enqueue_scripts() {
			...
			acf_localize_text(
				array(
					// 'Minimum values reached ( {min} values )' => __('Minimum values reached ( {min} values )', 'mytravel'),
					'Maximum values reached ( {max} values )' => __( 'Maximum values reached ( {max} values )', 'mytravel' ),
					'Loading'          => __( 'Loading', 'mytravel' ),
					'No matches found' => __( 'No matches found', 'mytravel' ),
				)
			);
		}
...
<div <?php echo acf_esc_attrs( $atts ); ?>>
	...
	<div class="filters -f<?php echo esc_attr( $filter_count ); ?>">
		...
		<div class="filter -search">
					...
					acf_text_input(
						array(
							'placeholder' => __( 'Search...', 'mytravel' ),
							'data-filter' => 's',
						)
					);
					?>
		...
		*/
		function render_field_settings( $field ) {
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Filter by Post Type', 'mytravel' ),
					'instructions' => '',
					'type'         => 'select',
					'name'         => 'post_type',
					...
					'ui'           => 1,
					'allow_null'   => 1,
					'placeholder'  => __( 'All post types', 'mytravel' ),
				)
			);
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Filter by Post Status', 'mytravel' ),
					'instructions' => '',
					'type'         => 'select',
					'name'         => 'post_status',
					...
					'ui'           => 1,
					'allow_null'   => 1,
					'placeholder'  => __( 'Any post status', 'mytravel' ),
				)
			);
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Filter by Taxonomy', 'mytravel' ),
					'instructions' => '',
					'type'         => 'select',
					'name'         => 'taxonomy',
					...
					'ui'           => 1,
					'allow_null'   => 1,
					'placeholder'  => __( 'All taxonomies', 'mytravel' ),
				)
			);
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Filters', 'mytravel' ),
					'instructions' => '',
					'type'         => 'checkbox',
					'name'         => 'filters',
					'choices'      => array(
						'search'    => __( 'Search', 'mytravel' ),
						'post_type' => __( 'Post Type', 'mytravel' ),
						'taxonomy'  => __( 'Taxonomy', 'mytravel' ),
					),
				)
			);
			...
			acf_render_field_setting(
				...
				array(
					...
					'name'         => 'return_format',
					'choices'      => array(
						'object' => __( 'Post Object', 'mytravel' ),
						'id'     => __( 'Post ID', 'mytravel' ),
					),
					'layout'       => 'horizontal',
				)
			);
		...
		function render_field_validation_settings( $field ) {
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Minimum Posts', 'mytravel' ),
					'instructions' => '',
					'type'         => 'number',
					'name'         => 'min',
				)
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Maximum Posts', 'mytravel' ),
					'instructions' => '',
					'type'         => 'number',
					'name'         => 'max',
				)
		...
		function render_field_presentation_settings( $field ) {
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Elements', 'mytravel' ),
					'instructions' => __( 'Selected elements will be displayed in each result', 'mytravel' ),
					'type'         => 'checkbox',
					'name'         => 'elements',
					'choices'      => array(
