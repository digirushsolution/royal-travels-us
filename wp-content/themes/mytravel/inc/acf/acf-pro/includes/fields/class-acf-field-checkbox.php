	...
	class acf_field_checkbox extends acf_field {
		...
		*/
		...
		function initialize() {
			...
			// vars
			$this->name          = 'checkbox';
			$this->label         = __( 'Checkbox', 'mytravel' );
			$this->category      = 'choice';
			$this->description   = __( 'A group of checkbox inputs that allow the user to select one, or multiple values that you specify.', 'mytravel' );
			...
			$this->defaults      = array(
				...
				'toggle'                    => 0,
				'return_format'             => 'value',
				'custom_choice_button_text' => __( 'Add new choice', 'mytravel' ),
			);
		}
		...
		function validate_value( $valid, $value, $field, $input ) {
			...
			foreach ( $value as $value ) {
				if ( empty( $value ) && $value !== '0' ) {
					return __( 'Checkbox custom values cannot be empty. Uncheck any empty values.', 'mytravel' );
				}
			}
		...
		*/
		function render_field_settings( $field ) {
			...
			acf_render_field_setting(
				...
				array(
					...
					'instructions' => __( 'Specify the returned value on front end', 'mytravel' ),
					'type'         => 'radio',
					'name'         => 'return_format',
					'layout'       => 'horizontal',
					'choices'      => array(
		...
		function render_field_validation_settings( $field ) {
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Allow Custom Values', 'mytravel' ),
					'name'         => 'allow_custom',
					'type'         => 'true_false',
					'ui'           => 1,
					'instructions' => __( "Allow 'custom' values to be added", 'mytravel' ),
				)
			);
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Save Custom Values', 'mytravel' ),
					'name'         => 'save_custom',
					'type'         => 'true_false',
					'ui'           => 1,
					'instructions' => __( "Save 'custom' values to the field's choices", 'mytravel' ),
					'conditions'   => array(
						'field'    => 'allow_custom',
						'operator' => '==',
						'value'    => 1,
		...
		function render_field_presentation_settings( $field ) {
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Layout', 'mytravel' ),
					'instructions' => '',
					'type'         => 'radio',
					'name'         => 'layout',
					'layout'       => 'horizontal',
					'choices'      => array(
						'vertical'   => __( 'Vertical', 'mytravel' ),
						'horizontal' => __( 'Horizontal', 'mytravel' ),
					),
				)
			);
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Add Toggle All', 'mytravel' ),
					'instructions' => __( 'Prepend an extra checkbox to toggle all choices', 'mytravel' ),
					'name'         => 'toggle',
					'type'         => 'true_false',
					'ui'           => 1,
				)
