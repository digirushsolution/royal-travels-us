...
if ( ! class_exists( 'ACF_Field_User' ) ) :
	...
	class ACF_Field_User extends ACF_Field {
		...
		function initialize() {
			$this->name          = 'user';
			$this->label         = __( 'User', 'mytravel' );
			$this->category      = 'relational';
			$this->description   = __( 'Allows the selection of one or more users which can be used to create relationships between data objects.', 'mytravel' );
		...
		function render_field_settings( $field ) {
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Filter by Role', 'mytravel' ),
					'instructions' => '',
					'type'         => 'select',
					'name'         => 'role',
					...
					'ui'           => 1,
					'allow_null'   => 1,
					'placeholder'  => __( 'All user roles', 'mytravel' ),
				)
			);
			...
			acf_render_field_setting(
				...
				array(
					...
					'name'         => 'return_format',
					'choices'      => array(
						'array'  => __( 'User Array', 'mytravel' ),
						'object' => __( 'User Object', 'mytravel' ),
						'id'     => __( 'User ID', 'mytravel' ),
					),
					'layout'       => 'horizontal',
				)
			);
		...
		function ajax_query_init( $request, $query ) {
			...
			if ( ! $query->field || $query->field['type'] !== $this->name ) {
				$query->send( new WP_Error( 'acf_missing_field', __( 'Error loading field.', 'mytravel' ), array( 'status' => 404 ) ) );
			}
			...
			if ( ! isset( $_REQUEST['user_query_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( $_REQUEST['user_query_nonce'] ), 'acf/fields/user/query' . $query->field['key'] ) ) {
				$query->send( new WP_Error( 'acf_invalid_request', __( 'Invalid request.', 'mytravel' ), array( 'status' => 404 ) ) );
			}
		}
		...
		public function validate_rest_value( $valid, $value, $field ) {
			...
			if ( count( $invalid_users ) ) {
				$error         = sprintf(
					__( '%1$s must have a valid user ID.', 'mytravel' ),
					$param
				);
