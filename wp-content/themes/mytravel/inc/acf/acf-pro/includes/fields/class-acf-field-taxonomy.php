...
if ( ! class_exists( 'acf_field_taxonomy' ) ) :
	...
	class acf_field_taxonomy extends acf_field {
		...
		public function initialize() {
			$this->name          = 'taxonomy';
			$this->label         = __( 'Taxonomy', 'mytravel' );
			$this->category      = 'relational';
			$this->description   = __( 'Allows the selection of one or more taxonomy terms based on the criteria and options specified in the fields settings.', 'mytravel' );
	...
	<div class="acf-actions -hover">
		...
		*/
		...
		public function render_field_checkbox( $field ) {
			...
			$args = array(
				'taxonomy'         => $field['taxonomy'],
				'show_option_none' => sprintf( _x( 'No %s', 'No Terms', 'mytravel' ), $taxonomy_obj->labels->name ),
				'hide_empty'       => false,
				'style'            => 'none',
		...
		*/
		function render_field_settings( $field ) {
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Taxonomy', 'mytravel' ),
					'instructions' => __( 'Select the taxonomy to be displayed', 'mytravel' ),
					'type'         => 'select',
					'name'         => 'taxonomy',
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Create Terms', 'mytravel' ),
					'instructions' => __( 'Allow new terms to be created whilst editing', 'mytravel' ),
					'name'         => 'add_term',
					'type'         => 'true_false',
					'ui'           => 1,
				)
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Save Terms', 'mytravel' ),
					'instructions' => __( 'Connect selected terms to the post', 'mytravel' ),
					'name'         => 'save_terms',
					'type'         => 'true_false',
					'ui'           => 1,
				)
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Load Terms', 'mytravel' ),
					'instructions' => __( 'Load value from posts terms', 'mytravel' ),
					'name'         => 'load_terms',
					'type'         => 'true_false',
					'ui'           => 1,
				)
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Return Value', 'mytravel' ),
					'instructions' => '',
					'type'         => 'radio',
					'name'         => 'return_format',
					'choices'      => array(
						'object' => __( 'Term Object', 'mytravel' ),
						'id'     => __( 'Term ID', 'mytravel' ),
					),
					'layout'       => 'horizontal',
				)
			);
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Appearance', 'mytravel' ),
					'instructions' => __( 'Select the appearance of this field', 'mytravel' ),
					'type'         => 'select',
					'name'         => 'field_type',
					'optgroup'     => true,
					'choices'      => array(
						__( 'Multiple Values', 'mytravel' ) => array(
							'checkbox'     => __( 'Checkbox', 'mytravel' ),
							'multi_select' => __( 'Multi Select', 'mytravel' ),
						),
						__( 'Single Value', 'mytravel' ) => array(
							'radio'  => __( 'Radio Buttons', 'mytravel' ),
							'select' => _x( 'Select', 'noun', 'mytravel' ),
						),
					),
				)
			);
		...
		*/
		...
		function ajax_add_term() {
			...
			if ( ! current_user_can( $taxonomy_obj->cap->manage_terms ) ) {
				wp_send_json_error(
					array(
						'error' => sprintf( __( 'User unable to add new %s', 'mytravel' ), $taxonomy_label ),
					)
				);
			}
			...
			if ( $args['term_name'] ) {
				...
				if ( term_exists( $args['term_name'], $field['taxonomy'], $args['term_parent'] ) ) {
					wp_send_json_error(
						array(
							'error' => sprintf( __( '%s already exists', 'mytravel' ), $taxonomy_label ),
						)
					);
				}
				...
				wp_send_json_success(
					array(
						'message'     => sprintf( __( '%s added', 'mytravel' ), $taxonomy_label ),
		...
		<form method="post">
			...
			?>
		<p class="acf-submit">
			<button class="acf-submit-button button button-primary" type="submit"><?php _e( 'Add', 'mytravel' ); ?></button>
		</p>
		</form><?php
