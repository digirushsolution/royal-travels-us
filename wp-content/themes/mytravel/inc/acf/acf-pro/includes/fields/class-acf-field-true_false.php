	...
	class acf_field_true_false extends acf_field {
		...
		*/
		...
		function initialize() {
			...
			$this->label         = __( 'True / False', 'mytravel' );
			$this->category      = 'choice';
			$this->description   = __( 'A toggle that allows you to pick a value of 1 or 0 (on or off, true or false, etc). Can be presented as a stylized switch or checkbox.', 'mytravel' );
		...
		*/
		...
		function render_field( $field ) {
			...
			if ( $field['ui'] ) {
				...
				// vars
				if ( $field['ui_on_text'] === '' ) {
					$field['ui_on_text'] = __( 'Yes', 'mytravel' );
				}
				if ( $field['ui_off_text'] === '' ) {
					$field['ui_off_text'] = __( 'No', 'mytravel' );
				}
		...
		*/
		function render_field_settings( $field ) {
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Message', 'mytravel' ),
					'instructions' => __( 'Displays text alongside the checkbox', 'mytravel' ),
					'type'         => 'text',
					'name'         => 'message',
				)
			);
		...
		function render_field_presentation_settings( $field ) {
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'On Text', 'mytravel' ),
					'instructions' => __( 'Text shown when active', 'mytravel' ),
					'type'         => 'text',
					'name'         => 'ui_on_text',
					'placeholder'  => __( 'Yes', 'mytravel' ),
					'conditions'   => array(
						'field'    => 'ui',
						'operator' => '==',
						'value'    => 1,
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Off Text', 'mytravel' ),
					'instructions' => __( 'Text shown when inactive', 'mytravel' ),
					'type'         => 'text',
					'name'         => 'ui_off_text',
					'placeholder'  => __( 'No', 'mytravel' ),
					'conditions'   => array(
						'field'    => 'ui',
						'operator' => '==',
						'value'    => 1,
