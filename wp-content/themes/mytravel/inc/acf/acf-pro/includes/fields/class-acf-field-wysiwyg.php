	...
	class acf_field_wysiwyg extends acf_field {
		...
		*/
		...
		function initialize() {
			...
			// vars
			$this->name          = 'wysiwyg';
			$this->label         = __( 'WYSIWYG Editor', 'mytravel' );
			$this->category      = 'content';
			$this->description   = __( 'Displays the WordPress WYSIWYG editor as seen in Posts and Pages allowing for a rich text-editing experience that also allows for multimedia content.', 'mytravel' ) . ' ' . __( 'We do not recommend using this field in ACF Blocks.', 'mytravel' );
			...
			<div id="wp-<?php echo esc_attr( $id ); ?>-editor-tools" class="wp-editor-tools hide-if-no-js">
				...
				<?php endif; ?>
					...
					<div class="wp-editor-tabs">
						<button id="<?php echo esc_attr( $id ); ?>-tmce" class="wp-switch-editor switch-tmce" data-wp-editor-id="<?php echo esc_attr( $id ); ?>" type="button"><?php echo __( 'Visual', 'mytravel' ); ?></button>
						<button id="<?php echo esc_attr( $id ); ?>-html" class="wp-switch-editor switch-html" data-wp-editor-id="<?php echo esc_attr( $id ); ?>" type="button"><?php echo _x( 'Text', 'Name for the Text editor tab (formerly HTML)', 'mytravel' ); ?></button>
					</div>
				<?php endif; ?>
			</div>
			<div id="wp-<?php echo esc_attr( $id ); ?>-editor-container" class="wp-editor-container">
					<?php if ( $field['delay'] ) : ?>
					<div class="acf-editor-toolbar"><?php _e( 'Click to initialize TinyMCE', 'mytravel' ); ?></div>
				<?php endif; ?>
		...
		*/
		function render_field_settings( $field ) {
			acf_render_field_setting(
				...
				array(
					...
					'instructions' => __( 'Appears when creating a new post', 'mytravel' ),
					'type'         => 'textarea',
					'name'         => 'default_value',
				)
			);
		...
		function render_field_presentation_settings( $field ) {
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Tabs', 'mytravel' ),
					'instructions' => '',
					'type'         => 'select',
					'name'         => 'tabs',
					'choices'      => array(
						'all'    => __( 'Visual & Text', 'mytravel' ),
						'visual' => __( 'Visual Only', 'mytravel' ),
						'text'   => __( 'Text Only', 'mytravel' ),
					),
				)
			);
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Show Media Upload Buttons', 'mytravel' ),
					'instructions' => '',
					'name'         => 'media_upload',
					'type'         => 'true_false',
					'ui'           => 1,
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Delay Initialization', 'mytravel' ),
					'instructions' => __( 'TinyMCE will not be initialized until field is clicked', 'mytravel' ),
					'name'         => 'delay',
					'type'         => 'true_false',
					'ui'           => 1,
					'conditions'   => array(
