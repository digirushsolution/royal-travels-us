...
function acf_register_location_type( $class_name ) {
	...
	if ( ! class_exists( $class_name ) ) {
		$message = sprintf( __( 'Class "%s" does not exist.', 'mytravel' ), $class_name );
	...
	if ( $store->has( $name ) ) {
		$message = sprintf( __( 'Location type "%s" is already registered.', 'mytravel' ), $name );
		_doing_it_wrong( __FUNCTION__, $message, '5.9.0' );
		return false;
	}
...
function acf_get_location_rule_types() {
	...
	$categories = array(
		...
		'user'  => __( 'User', 'mytravel' ),
		'forms' => __( 'Forms', 'mytravel' ),
	);
